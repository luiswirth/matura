\documentclass[12pt, a4paper]{article}

\usepackage[T1]{fontenc}
\usepackage[german]{babel}

% latex-meta
% creating new environemnts more convenient
\usepackage{environ}

% for formulas
\usepackage{amsmath}
\usepackage{amsfonts}
\usepackage{amssymb}
\usepackage[amssymb]{SIunits}
\usepackage{mathtools}
\usepackage{esvect} % for vector typesetting
\usepackage{bm} % for matrix typesetting
\usepackage{xfrac}

% for bibliography
\usepackage[
backend=biber,
citestyle=authortitle
%style=science
]{biblatex}
\addbibresource{references.bib}
\usepackage{csquotes}


% for graphics
\usepackage{graphicx}
\graphicspath{{images/}{../images/}}
\usepackage{wrapfig} % for Text around images

% for drawing
\usepackage{tikz}
\usetikzlibrary{matrix,chains,positioning,calc,intersections,shapes,arrows,fadings}
\usepackage{tikzpagenodes}
\usepackage{pgfplots}

% for colors
\usepackage{xcolor}

% for the multifile structure
\usepackage{subfiles}

% for page layout
\usepackage{geometry}
\geometry{a4paper, portrait, margin=2cm}

% footer and header
\usepackage{fancyhdr}
\pagestyle{fancy}
\fancyhf{}
\renewcommand{\headrulewidth}{2pt}
\rhead{Luis Wirth}
\lhead{Maturarbeit}
\chead{Machine Learning}
\cfoot{\thepage}

% footnote at bottom
\usepackage[bottom]{footmisc}

\usepackage{background}

\backgroundsetup{
contents={}
}


% misc
\usepackage{url}

% paragraph numbering and formatting
\usepackage{titlesec}
\setcounter{secnumdepth}{4}

\titleformat{\paragraph}
{\normalfont\normalsize\bfseries}{\theparagraph}{1em}{}
\titlespacing*{\paragraph}
{0pt}{3.25ex plus 1ex minus .2ex}{1.5ex plus .2ex}

% disable pargraph indentation ----- GOOD???
\setlength{\parindent}{0pt}

\colorlet{bgcolor}{white}
\colorlet{textcolor}{black}
\pagecolor{bgcolor}
\color{textcolor}

%------------------------------------------------

\begin{document}

\input{sections/utils.tex} % including utils.tex

\subfile{sections/titelblatt}
\pagebreak

\tableofcontents
\pagebreak

\subfile{sections/vorwort}
\pagebreak

\subfile{sections/einleitung}
\pagebreak

\subfile{sections/theorie}
\pagebreak

\subfile{sections/entwicklung}
\pagebreak

\subfile{sections/diskussion}
\pagebreak

\subfile{sections/schlusswort}
\pagebreak

\printbibliography[heading=bibintoc]
\pagebreak

\listoffigures
\pagebreak

\listoftables
\pagebreak

\section*{Selbstständigkeitserklärung}
Ich erkläre hiermit, dass ich diese Arbeit selbständig durchgeführt und keine anderen als die angegebene Quellen, Hilfsmittel und Hilfspersonen beigezogen habe. Alle Textstellen in der Arbeit, die wörtlich oder sinngemäss aus Quellen entnommen wurden, habe ich als solche gekennzeichnet.

\vspace{2cm}
\begin{center}
    \noindent\rule{5cm}{0.4pt}\\
    Luis Wirth
\end{center}

\end{document}

%%% Local Variables:
%%% mode: latex
%%% TeX-master: t
%%% End:
