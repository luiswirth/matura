...
\noindent \begin{tikzpicture}
     \node [mybox] (box) {%
             ...
             Der Gradient $\vec{\nabla}f(\vec{p})$ einer Funktion $f$ bei einem Punkt $\vec{p}$ ist ein Vektor, welcher in die Richtung des steilsten Anstieg von $f$ zeigt.
...
$\vec{p}_{z+1} = \vec{p}_z - \eta \cdot \vec{\nabla} \mathit{f}(\vec{p}_z)$, wobei $\eta$ die Schrittgroesse festlegt.
