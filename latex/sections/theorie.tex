...
Der \keyword{Trainingsdatensatz} $\set{X}$ beinhaltet gewisse \keyword{Attribute}, die Merkmalstypen der Inputdaten.
...
\subsubsection{Filter als Faltungen}
\paragraph{Graustufige Bilder}
...
Zuerst einmal stellt sich die Frage, weshalb sich Filter eignen fuer Machine
Learning.
Wie bereits erwaehnt besteht die Aufgabe von Filtern darin bestimmte Features
eines Bildes hervorzuheben und die restlichen auszublenden. Dies ist der gleiche
Gedanke wie bei einem normalen KNN mit Sigmoiden Neuronen, welche dann auch
erlernen auf gewisse Features zu reagieren.
Jedoch ist ein wichtiger Unterschied von Filtern im Gegensatz zu normalen
Neuronen, dass sie Invarianzen aufweisen.
...
Filter werden erlernt. Elementeintraege sind Modellparameter. Filtergroesse bzw.
Groesse des Rezeptiven Feldes ist Hyperparameter.
\begin{equation*}
  \mat{F} = \begin{pmatrix}
    w_{1,1} & \cdots & w_{1,f} \\
    \vdots & \ddots & \vdots \\
