...
Informatik hat mehrere Inputs und einen einzigen Output. Es besitzt ausserdem
mehrere \textbf{Gewichte} $w_1, \cdots, w_n$.
Die Gewichtete Summe der Inputs mit den Gewichten
...
Der Output des Perzeptron kann nur entweder 0 oder 1 sein. Falls die Gewichtete
Summe groesser als der Schwellwert ist, betraegt er 1 ansonsten 0. Siehe erster Teil
der Gleichung (\ref{eq:perzeptron_1}).
Es ist gaengig die Ungleichung der Bedingung in die Normalform zu bringen und $\tilde{b}$ mit
$b = -\tilde{b}$ zu ersetzten. Somit lautet die Ungleichung: $\tilde{z} + b
> 0$. Der Term $\tilde{z} + b$ wird mit $z$ bezeichnet.
...
Hier nochmals alles in einer Gleichung zusammengefasst.
\begin{equation}
  p(\vec{x}) =
  \begin{cases}
...
In Abbildung (\ref{fi:perzeptron}) ist ein Perzeptron mit drei Inputs dargestellt.
...
Schichten: Input, Zwischen(Deep), Output; Feedforward
