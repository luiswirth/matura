...
erlernen. Zuerst werden wir 2D-Filter betrachten, welche sich fuer graustufige Bilder eignen.
\para
Filter sind einfach Regionen, deutlich kleiner als das verarbeitete Bild, welche
ueber alle Pixel wandert und diese manipuliert und so wieder ein neues Bild liefert.
Mathematisch gesehen handelt es sich bei einem Filter einfach um eine Matrix,
die sogennante \keyword{Filtermatrix} (siehe Abb. (\ref{fig:filtermatrix})),
bezeichnet mit $\mat{F}$. Die Elemente von ihr bestimmen das
...
\end{figure}
\para
Als Beispiel nehmen wir eine Filtermatrix $\mat{F}_{3 \times 3}$ der Form $3 \times 3$.
Meistens beginnt der Filter oben links ueber das Bild $\mat{B}$ zu wandern.
Wenn der Filter ueber der $3 \times 3$-Region des Bildes liegt, ist jedem
Element des Filters ein Element der Bildmatrixregion $\tilde{\mat{B}}$ zugeordnet. Nun wird
das Hadamard-Produkt (das elementweise Produkt) des Filters mit der
Bildmatrixregion gebildet, welche eine neue Matrix $\mat{P}_{3 \times 3} = \mat{F} \odot
\tilde{\mat{B}}$ ist. Nun werden alle Elemente der neuen Matrix $\mat{P}$
aufaddiert. Diese Zahl ist dann der Grauwert des ersten Pixels des bearbeiteten
Bildes $\mat{B}^*$.
\begin{equation*}
  (\mat{B}^*)_{1,1} = \sum_{i=1}^3 \sum_{j=1}^3 (\mat{P})_{i,j}
\end{equation*}
...
\end{figure}
\para
Ein Filter kann auch andere Masse haben als $3 \times 3$. Jedoch ist ein Filter $\mat{F}_{f \times f}$
immer quadratisch und seine Seitenlaenge, typischerweise mit $f$ bezeichent, sollte auch immer eine ungerade Zahl
sein.
\para
Hier nochmals die Schritte mathematisch fuer allgemeine Masse zusammengefasst:
\begin{equation}
  \mat{P}_{f \times f} = \mat{F}_{f \times f} \odot \tilde{\mat{B}}_{f \times f}
\end{equation}

\begin{equation}
  (\mat{B}_{h \times w}^*)_{1,1} = \sum_{i=1}^f \sum_{j=1}^f (\mat{P})_{i,j}
\end{equation}
NOCH ALLGEMEINERE GLEICHUNG -> BESCHRIFTUNG (REGION ANGEBEN)
\para

Um den Effekt eines Filters auf ein Bild zu zeigen, wird nun ein spezieller
Filter, der Sobel-Filter erklaert.
\begin{figure}[h!]
