...
\begin{equation}\label{eq:gradientdescent}
  \vec{p}_{t+1} = \vec{p}_t - \eta \cdot \vec{\nabla} \mathit{f}(\vec{p}_t)
\end{equation}
...
Nun kann ganz einfach die vektorisierte Aktivierungsfunktion $\varphi$ auf
$\vec{z}^{l+1}$ angewandt werden.

\begin{equation}\label{eq:aktivierung_matrix}
  \vec{a}^{l+1} = \varphi \left[\mat{W}^{l} \vec{a}^{l} + \vec{b}^{l} \right] = \varphi \left[ \vec{z}^{l+1} \right]
\end{equation}
...
WEITERE ERLAUETERUNGEN? SATURATION DER NEURONEN VERHINDERN DESHALB VARIANZ_{TOT}
NUR 1
...
Hierfuer ist es sinnvoll eine Zwischenwert, welchen wir Fehler
$\delta_j^l$ nennen.
