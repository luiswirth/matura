...
\begin{figure}[h!]
  ...
  \begin{tikzpicture}
    ...
    \draw[red] (0,0) arc (180:90:1cm);
    \coordinate (A) at (2,1);
    \fill[red] (A) circle (1mm);
    \coordinate (B) at (3,2);
    \fill[orange] (B) circle (2mm);

    \fill[blue] ($3*(A) - (B)$) circle (1mm);
    \draw[green] (0,0) parabola (1.5, 2.25);

    \draw[red,scale=0.3] plot[domain=-2:2] (\x,{\x*\x*\x });
  \end{tikzpicture}
...
verschiedensten Architekturen, welche je nach Problemstellung ausgewaehlt werden muessen.
...
kann. Des weiteren besitzt es mehrere sogenannte \textbf{Gewichte} $w_1, \ldots, w_n \in \mathbb{R}$.
Diese sind reele Zahlen, welche das Verhalten des Perzeptron erheblich beeinflussen.
Die \textbf{Gewichtete Summe} der Inputs mit den Gewichten
wird mit $\tilde{z}$ bezeichnet. Sie ist das
gleiche wie das Skalarprodukt des Gewichtevektor mit dem Inputvektor.\\
$\displaystyle \tilde{z} = \sum_{i=1}^{n} w_i x_i = \vec{w} \cdot \vec{x}$.\quad
...
Das Perzeptron verhaltet sich, dass falls die Gewichtete Summe groesser als der
Schwellwert ist, das Neuron feuert,
also betraegt der Output 1. Ansonsten ist er 0.
...
In Gleichung (\ref{eq:perzeptron_2}) ist nochmals alles zusammengefasst. Um das
Perzeptron zu trainiern, muesste einfach das Gradientenverfahren auf die
Gewichte und die Neigung angewendet werden.
\begin{equation}\label{eq:perzeptron_2}
  p(\vec{x}) =
  \begin{cases}
...
Was kann ein Perzeptron erlernen? -> Linearer Klassifikator
...
\subsubsection{Kuenstliche Neuronale Netze}
Wie das Perzeptron auch, besteht ein KNN aus Neuronen, in diesem Fall aber
mehreren. Es sind jedoch nicht ganz die gleichen Neuronen, denn fuer KNN
verwendet man sogennante \textbf{Sigmoide-Neuronen}, aber dazu spaeter mehr. Diese Neuronen sind in verschieden Schichten (engl.: Layers)
arangiert. Die erste ist die \textbf{Inputschicht}. Sie behinhalten die
Inputneuronen mit jeweils einem Inputwert. Dies sind eigentlich keine richtigen
Neuronen sondern eher Platzhalter fuer die Inputs. Als letztes kommt die
\textbf{Outputschicht} mit den Outputneuronen, welche jeweils einen Outputwert
besitzen. Dazwischen liegen die \textbf{Zwischenschichten} (engl.: Hiddenlayer). Von ihnen kann es
beliebig viele geben und in ihnen bliebig viele Neuronen.
Jedes Neuron aus einer Schicht ist mit jeden Neuron aus der naechsten Schicht ueber
...
Die soeben beschriebe Art von KNN nennt man \textbf{Feedforward-Netz}, da alle
Werte nur nach vorne propagiert werden.
...
\begin{figure}[h!]
  ...
  \begin{tikzpicture}
    ...
    \draw ($\j*(spacev)$) node[inner sep=0,minimum size=1cm,circle,draw] (l0n\j) {$x_{\j}$};
...
\subsubsection{Bessere Kuenstliche Neuronen}
...
$w_i$ assoziert. Zuerst wird die gewichtete Summe der Inputs $\tilde{z}$ gebildet.
...
\paragraph{Kuenstliche Neuronenen im Perzeptron}
...
annehmen koennen. Um das schwellenartige Verhalten des Perzeptron zu erhalten,
muss eine spezielle Aktivierungfunktion verwendet werden: die Heaviside-Funktion $\Theta$.
Sie ist eine Stufenfunktion mit einem einzigen Stufensprung bei $x=0$ vom Wert 0 auf 1.
...
Die Binaritaet des Perzeptron schrenkt es sehr in seinen Fahigkeiten ein.
Dadurch koennen sie auch nur fuer lineare Klassifizierungen verwednet werden.
Es ist deshalb sinnvoll eine Erweiterung auf stueckweise lineare Neuronen durchzufuehren.
Die Inputs muessen jetzt nicht mehr 0 oder 1 betragen, sondern koennen eine
beliebige Reele Zahlen zwischen 0 und 1 sein. Nun muss eine passende
Aktivierungsfunktion gewaehlt werden. Naheliegend waere eine lineare, welche
jedoch minimal 0 sein darf und maximal 1.
...
\begin{figure}[h!]
  ...
  \caption{die Formel und der Plot einer stueckweisen linearen Funktion}
\end{figure}


\paragraph{Sigmoide Neuronen}
Da KNN bestehend aus linearen Neuronen nur lineare Zusammenhaenge modellieren
koennen, waere es sinnvoll eine zweite Erweiterung auf nicht-lineare Neuronen zu machen.
Es gaengig dafuer sind sogennante Sigmoide Neuronen.
Sie erhalten ihren Namen, von ihrer Aktivierungsfunktion, der Sigmoid-Funktion $\sigma$.
Sie ist eine kontinuierliche Funktion mit einer Asymptote fuer $x \to -\infty$ mit $y=0$
und eine zweite fuer $x \to\infty$ mit $y=1$.
...
\begin{figure}[h!]
  ...
  \caption{die Formel und der Plot der Sigmoid-Funktion $\sigma$}
\end{figure}

\subsubsection{Vorwaertspropagierung}

Nun muessen einige Konventionen zur Beschriftung der Teil eines KNN getroffen
werden.
\begin{itemize}
  \item{$a_n^l$ ist die Aktivierung des $n$-ten Neurons in der $l$-ten Schicht.}
  \item{$w_{n,m}^l$ ist das Gewicht der Verbindung zweischen dem $n$-ten Neuron
      in der $l$-ten Schicht und dem $m$-ten Neuron in der $l+1$-ten Schicht. }
