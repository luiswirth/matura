...
\begin{itemize}
  ...
  \item{ 
    ...
    \item{Falls die Klammern in denen der Vektor $\vec{v}$ steht als eckige
        Klammern [] geschrieben sind, ist zu verstehene, dass die Funktion auf
        jeden der Vektorkomponenten angewandt wird.
        \begin{equation}
        f[\vec{v}]=
        \begin{pmatrix}
          f(v_1)\\
...
werden. Der Grund dafuer sollte spaeter einlauchtend sein.
\par\medskip
...
\subsubsection{Erweiterung der Kuenstlichen Neuronen}
...
genannten Aktivierungsfunktion, ueberall differenzierbar ist. Dies ist eine
Voraussetzung fuer das Gradientenverfahren und somit fuer das ganze Training.
Zugleich ist sie monoton steigend und das sehr glatt. 
Auch andere Funktion erfuellen diese Bedingungnen. Die Sogmoidfunktion wird
einfach bevorzusagt, da ihre Ableitung sehr simpel ist, siehe Abb. (\ref{fig:sigmoid}).
Ihre nicht-linearitaet ermoeglicht ein erlernen von deutlich komplexeren Sachverhalten.
Und wie spaeter erlautert, ist es moeglich mit der Kompostion von nicht-linearen
Funktionen jede belibige Funktion zu approximieren! 
\par\medskip
Die Simgmoid-Funktion besitzt eine einzige Wendestelle bei $x=0$ und hat zwei Asymptote, eine fuer $x \to -\infty$ mit $y=0$
...
Nun sollen diese Neuronen als Bausteine verwendet werden, um ein Kuenstliches
...
Um nun die Vorwaertspropagierung und somit die Berechnung der Outputs zu
erklaeren muessen einige Konventionen zur Bennung der Teile des KNN getroffen werden:
\begin{itemize}

\item{$n_j^l$ bezeichnet das $j$-te Neuron in der $l$-ten Schicht.}
\item{$z_j^l$ ist die Gewichtete Summe}
\item{$a_j^l$ ist die Aktivierung des $n$-ten Neurons in der $l$-ten Schicht.}
\item{$w_{j,k}^l$ ist das Gewicht der Verbindung vom $k$-ten Neuron
    in der $l-1$-ten Schicht zum $j$-ten Neuron in der $l$-ten Schicht. }
\item{$L$ soll die Anzahl der Schichten sein}

\end{itemize}

Da es nicht unueblich ist, dass ein KNN ueber 1000 Neuronen besitzt und ueber
10000 Gewichte ist es wichtig nicht den Ueberblick zu verlieren. Deshalb ist es
hilfreich mit Matrixen aus der lienaren Algebra zu arbeiten. + Performance GPU etc.

$\mathbf{w}$ ist die Gewichtsmatrix.
\subsubsection{Backpropagation}
Das Gradientenverfahren fuer KNN bezeichnet man als Backpropagation.
