...
\begin{figure}[h!]
  ...
  \begin{tikzpicture}
    ...
    \tikzstyle{netstyle} = [matrix of nodes,nodes={draw,circle,inner sep=0, minimum size=1cm},nodes in empty cell,column sep=2cm,row sep=-9pt]
    \tikzstyle{clear} = [draw=none,fill=none]

    \matrix[netstyle] (mat)
    {
      |[clear]| \parbox{1.3cm}{\centering Inputschicht}
      & |[clear]| \parbox{1.3cm}{\centering Zwischenschichten}
      & |[clear]| \parbox{1.3cm}{\centering Outputschicht} \\

      $x_{1}$  & |[clear]| & |[clear]|        & |[clear]| \\
      |[clear]| & $h_0^1$ & |[clear]| & |[clear]| \\
      $x_{2}$  & |[clear]| & $h_0^2$        & |[clear]| \\
      |[clear]|         & |[clear]|        & |[clear]| \phantom{$a_{0}^{0}$} \\
      $x_{3}$  & $\alpha_{1}^{1}$ & $\alpha_{0}^{2}$ \\
      |[clear]|         & |[clear]|        & |[clear]|  \phantom{$a_{0}^{0}$} \\
      $x_{4}$  & |[clear]|        & |[clear]| \\
      |[clear]|         & $\alpha_{2}^{1}$ & |[clear]| \\
      |[clear]| $\vdots$ & |[clear]|        & |[clear]| \\ 
    };
