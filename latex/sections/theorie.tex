...
Das Verhalten dieses Modells wird bestimmt durch seine \textbf{Modellparameter} $\lambda_1, \lambda_2,\ldots$.
...
Der Aufbau eines KNN bezeichnet man als \textbf{Topologie} des Netzes. Die
...
Zu guter letzt, wird noch eine \textbf{Gewichtsmatrix} $\mat{W}^l$ fuer jede
Schicht $l$ definiert.
...
Convolutional Neural Networks (CNN) (zu deutsch etwa: ``faltendes neuronales
Netz'') sind eine spezielle Architektur von einem KNN. Sie sind eine realtiv
moderne Architektur, welche in der Industire weit verbreitet ist. Ihre Staerke liegt
vorallem darin visuelle und autitive Daten zu verarbeiten. Sie sind sehr
effektiv in der Bildklassifizierung, Gesichtserkennung und Spracherkennung.

Wie bei dem Perzeptron und bei den KNNs ist die Architektur biologisch inspiriert.
...
CNNs sind im Grund wie normale KNNs. Sie haben jedoch noch weitere Arten von
Schichten mit spezifischen Funktionen, welche sich fuer Bildverarbeitung und
dergleichen eignet. Diese zusaetzlichen Schichten bestehen auch aus Neuronen,
diese funktioniere aber grundsaetzlich anderst als Sigmoideneuronen.
Ausserdem ist die Anordnung der Inputs bzw. der Neuronen anderst als bei den normalen KNNs, bei
welchen eine Schicht einfach eine 1-dimensionale Reihe von Neuronen ist.
Bei einem CNNs, welches graustufige Bilder verarbeitet, werden die Neuronen und ihre Werte
als 2-dimensionales Raster angeordnet. Dies ist sinnvoll, da die Pixel eines
Bildes, dessen Grauwerte den Input des Netzwerkes darstellen,
auch so angeordet sind.
Bei farbigen Bildern ist die Situation ein bisschen anderst. Dort besitzt jeder
Pixel drei Komponenten (R, G und B). Deshalb werden dann die Neuronen als ein 3-dimensionales Volumen angeordnet.

Daraus folgt, dass die Variabeln der Neuronen einer Schicht nicht mehr wie vorher als Vektoren
zusammengefasst werden koennen, sondern als 2-dimensionale (fuer schwarz-weiss
Bilder) bzw. 3-dimensionale (fuer farbige Bilder) Matrizen zusammengefasst
werden muessen.


---------------------
Der Input ist eine Tensor der Form $\text{Bildbreite} \times \text{Bildhoehe}
\times \text{Bildtiefe (Anzahl Komponenten eines Pixels)}$
--------------------
...
Wie vorhin erwaehnt, werden die Inputs als zwei-dimensionale Matrix betrachtet,
welche wir mit $\mat{X}$ bezeichnen. Dementsprechend sich auch die Neuronen
angeordnet. Die Aktivierungsmatrix wird mit $\mat{A}$ bezeichnet.
Die Aktivierung eines Neurons wird ueber eine diskrete Faltung (engl.
Convolution) berechnet.

Man kann sich das Verhalten einer Convolutional Schicht, wie einen Filter eines
Bildbearbeitungsprogramm, wie Photoshop, vorstellen.
...
gepoolten Bild. -> 75\% Informationsreduktion ``MaxPooling2D''.

\subsubsection{Fully-connected Layer}

\subsubsection{Eigenschaften von CNN}
...
Gleiches verhalten wie im visuellen cortex. 
...
\begin{gather}
  ...
  \displaystyle\phi,\psi = \operatorname*{arg\,min}_{\phi,\psi} \parallel {X - (\lambda \circ \psi)X \parallel}^2
\end{gather}
