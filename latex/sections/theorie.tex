...
besteht aus einem einzigen Kuenstlichen Neuron. Ein Kuenstliches Neuron in der
Informatik hat mehrere binaere Inputs und einen einzigen binaeren Output. Binaer
bedeutet, dass der Wert nur entweder 0 (\textit{aus}) oder 1 (\textit{ein}) sein
kann. Es besitzt ausserdem mehrere \textbf{Gewichte} $w_1, \cdots, w_n \in \mathbb{R}$.
...
Das Perzeptron besitzt ausserdem einen \textbf{Schwellenwert} $\tilde{b}$.
Zusammen mit den Gewichten, bildet er die Modellparameter.
Falls die Gewichtete Summe groesser als der Schwellwert ist, feuert das Neuron,
...
Mithilfe von $z$ kann der Begriff der \textbf{Aktivierung} $a$ eingefuehrt
werden. Sie ist das Resultat der \textbf{Aktivierungsfunktion} angewandt auf $z$.
$a = a(z).$ Um das Verhalten des Perzeptron zu erhalten muss man als
Aktivierungsfunktion die sogennante\\
Heaviside-Funktion $\Theta$ verwendet werden.
Sie ist eine Stufenfunktion mit einer einzigen Stufe bei $x=0$ von 0 auf 1.
...
\begin{figure}[h!]
  ...
  \begin{tikzpicture}
    ...
    \draw ($\j*(spacev)$) node[inner sep=0,minimum size=1cm,circle,draw] (n:0:\j) {$x_{\j}$};

    \foreach \l in {1,...,\hiddenlayers}
    \foreach \j in {1,...,\hiddens}
    \draw ($\j*(spacev) + \l*(spaceh) + (spaceb)$) node[inner sep=0,minimum size=1cm,circle,draw] (n:\l:\j) {$h_{\j}^{\l}$};

    \foreach \j in {1,...,\outputs}
    \draw ($\j*(spacev) + 2*(spaceb) + \hiddenlayers*(spaceh)$) node[inner sep=0,minimum size=1cm,circle,draw] (n:0:\j) {$y_{\j}$};
...
Tiefe Neuronalsnetze -> viele Hiddenlayers -> Deep Learning.
