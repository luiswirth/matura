...
\noindent \begin{tikzpicture}
     \node [mybox] (box) {%
             ...
             Der Gradient $\vec{\nabla}f(\vec{p})$ einer Funktion $f$ in einem Punkt $\vec{p}$ ist ein Vektor, welcher in die Richtung des steilsten Anstiegs von $f$ zeigt.
             Sein Magnitude ist ein Mass fuer die Starke der Zunahme. Somit ist er eine Erweiterung der Ableitung auf $n$-dimensionale Funktionen.\\
            ...
            Bei den $\frac{\partial f}{\partial x_i}$ handelt es sich um partielle Ableitungen.
...
\noindent \begin{tikzpicture}
    \node [mybox] (box) {%
            \begin{minipage}{1.0\textwidth}
                ...
                $\displaystyle\frac{\partial f}{\partial x_i}(a) := \lim_{h \to 0} \frac{f(a_1,\cdots,a_i + h,\cdots,a_n)-f(a_1,\cdots,a_i,\cdots,a_n)}{h}$
...
Somit ist der Vektor $\vec{b}_z$ einfach ein skalierter negativer Gradient der Funktion $f$ im Punkt $\vec{p}_z$.
Also kann jetzt das iterative Annahern des lokalen Minimums so beschreiben werden:\\
$\displaystyle\vec{p}_{z+1} = \vec{p}_z - \eta \cdot \vec{\nabla} \mathit{f}(\vec{p}_z)$,\\
wobei $\eta$ die Schrittgroesse festlegt.
