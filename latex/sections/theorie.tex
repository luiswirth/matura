...
\begin{itemize}
  ...
  \item{ 
    ...
    \item{Im Allgemeinen ist die grossgeschriebene Variante $J$ eines Indizes
...
\begin{tikzpicture}
  \node [mybox] (box) {%
    \begin{minipage}{1.0\textwidth}
      ...
      \begin{equation*}
        \frac{\partial f}{\partial x_i}(\vec{a}) := \lim_{h \to 0} \frac{f(a_1,\cdots,a_i + h,\cdots,a_n)-f(a_1,\cdots,a_i,\cdots,a_n)}{h}
      \end{equation*}
...
\item{$w_{j,k}^l$ (man beachte die vertauschten Indizes) ist das Gewicht der Verbindung vom $k$-ten Neuron
    ...
    \footnote{
      ...
      Sinn fuer die spaeteren mathematischen Operationen mit Matrizen.
}}
...
Um nun lineare Algebra ins Spiel zu bringen, wird eine Gewichtsmatrxi
$\mathbf{w}^l$ fuer jede Schicht $l$ definiert.
...
\begin{equation}\label{eq:gewichtsmatrix}
  \mathbf{w}^l =
  \begin{pmatrix}
    w_{1,1}^l & w_{1,2}^l & \cdots & w_{1,K}^l \\[0.3em]
    w_{2,1}^l & w_{2,2}^l & \cdots & w_{2,K}^l \\[0.3em]
    \vdots & \vdots & \ddots & \vdots \\[0.3em]
    w_{J,1}^l & w_{J,2}^l & \cdots & w_{J,K}^l
  \end{pmatrix}
\end{equation}
...
\begin{tikzpicture}
  ...
  \node[fancytitle,right=10pt] at (box.north west) {Vektoriserung einer Funktion};
\end{tikzpicture}
\par\medskip
...
\begin{equation}\label{eq:activierung_matrix}
  \vec{a}^{\,l} = \sigma \left(\mathbf{w}^{\,l} \vec{a}^{\,l-1} + \vec{b}^{\,l} \right)
\end{equation}
...
\begin{tikzpicture}
  ...
  \node[fancytitle,right=10pt] at (box.north west) {Das Hadamard-Produkt};
\end{tikzpicture}
\par\medskip
