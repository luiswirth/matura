...
Der Themenbereich des \keyword{Maschinellen Lernens} beschäftigt sich mit Algorithmen und mathematischen Modellen, welche von selber lernen, Probleme zu lösen.
...
gewünschten Zielwerte sind. Die \keyword{Vorhersagen} sind die Outputs, welche
...
\para{}
Für diese Optimierung wird das sogennante \keyword{Gradientenverfahren} (engl.: Gradient descent) verwendet.
...
\para{}
Die sogennante \keyword{Lernrate} $\eta$ aus Gleichung (\ref{eq:gd_ml}) stellt
...
\end{figure}
In den Anfängen des Maschinellen Lernens verwendet man einfach die normierte
...
\begin{figure}[h!]
  \centering
  \includegraphics[width=0.3\textwidth]{filter.pdf}
...
\begin{figure}[h!]
  \centering
  \includegraphics[width=0.9\textwidth]{conv.pdf}
...
\begin{figure}[h!]
  \centering
  \includegraphics[width=0.5\textwidth]{ae1.pdf}
...
Labels \code{y} mithilfe der Wegwerf-Variabel \code{\_} verworfen.
\begin{minted}[frame=lines,framesep=2mm,baselinestretch=1.2,bgcolor=lightgray,fontsize=\footnotesize,linenos]{python}
...
\end{minted}
Das Modell hat nach dem Training einen \code{val\_los} von $0.1089$,
...
\subsubsection{TensorBoard Konfiguration}
...
werden. Wichtig ist die Präzision des Modells. Die entsprechende Option wird \code{accuracy} genannt.
\begin{minted}[frame=lines,framesep=2mm,baselinestretch=1.2,bgcolor=lightgray,fontsize=\footnotesize,linenos]{python}
