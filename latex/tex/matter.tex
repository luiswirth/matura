...
$(f,g)$, ueber einen $d$-dimensionalen Eingaberaum $X \subseteq \set{R}^d$. Die
Elemente des Eingaberaums sind einfach die Features $\vec{x} \in X$.
Neben dem Eingaberaum existiert noch ein Encodingraum $E$, welcher
niedrig-diemensionale Codierung darstellt.
Gesucht werden
nun die Funktionen
\begin{itemize}
\item{$f: X \to E$ (Encoder-Funktion)}
\item{$g: F \to X$ (Decoder-Funktion)}
\end{itemize}
wobei $F \subseteq \set{R}^p$ ein $p$-dimensionaler Zwischenraum (Encoding-Repraesentation) mit $p \ll d$
ist und fuer alle $x \in X$ gilt:
\begin{equation}
  |x - (g \circ f)(x)| \to \text{min}
\end{equation}
Bedeutung: Bei Anwendung der Funktion $g$ auf das Ergebniss der Anwendung von
$f$ auf ein beliebiges Elemente $x$ des Eingaberaums, optimalerweise genau das
urspruengliche Element $\hat{x} = x$ erhaelt. Dieses Verhalten wird
durch die Minimierung des Fehlers zwischen $x$ und $\hat{x}$ gefordert. Da $p$
kleiner als $d$ ist muss $F$ offentsichtlich eine niedrig-dimensionale Codierung
von $X$ darstellen.
$\hat{x} = (g \circ f)(x)$
Optimal: $\hat{x} = x$ aber nicht moeglich, daher eigentlich $g: F \to D$
