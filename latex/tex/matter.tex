...
\begin{figure}[h!]
  ...
  \begin{tikzpicture}[
    ...
    \begin{axis}[
      ...
      \node[fill=red,circle,minimum size=0.15cm,inner sep=0,outter sep=0] (wp1) at (-1,{gausseval(0,1,-1)}) {};
      \node[fill=red,circle,minimum size=0.15cm,inner sep=0,outter sep=0] (wp2) at (1,{gausseval(0,1,1)}) {};
...
genau zu sein betrachten wir Tensorflow-Core, denn TF als ganzes ist eine
riesige API. Zuerst
wie man TF zu verwenden hat, anhand gewisser Konzepte und danach noch die
zugrundeliegene Implementation.
...
\end{infobox}
\para{}
Der Client - also das Frontend - von TF ist in Python geschrieben.
\footnote{
...
TensorFlow zu verwenden.
\para{}
Der Master - also das Backend - von TF ist dagegen in C++ (und CUDA)
...
spaeter ein ML-Modell damit basteln koennen. Dafuer muessen
wir den Client von TF betrachten. Man verwendet verschiedene Konzepte um
programmatisch mit TF zu arbeiten. Im folgenden Abschnitt werden wir diese
Konzepte behandeln.

\subsubsection{Grundideen}
MOEGLICHER AUFBAU
Tensoren, Graph (mit Operation), Session


The zentrale Dateneinheit in TF sind tensoren. Rank = Grad und Shape = Form.

\subsubsection{Konzepte}
\paragraph{Graph}
TensorFlow ist datenstromorientiert. Das bedeutet, dass man eine Berechnung
(engl.: computation) durch einen gerichteten Graphen beschreibt. Diesen Graphen
stellt man zuerst durch eine Definition auf und erst nach der Errichtung wird er
dann Knoten fuer Knoten ausgefuehrt.
Bei diesen Graphen handelt es sich um \keyword{Computation Graphs}, wie wir sie schon in Sektion (\ref{sec:backpropagation}) kennengelernt
...
\para{}
In Abbildung () ist der Python-Code zur Kreiirung und Zusammenbau

[von tensorflow.org/apidocs: A Graph contains a set of tf.Operation objects, which represent units of computation; and tf.Tensor objects, which represent the units of data that flow between operations.

A default Graph is always registered, and accessible by calling tf.compat.v1.getdefaultgraph. To add an operation to the default graph, simply call one of the functions that defines a new Operation]

there exists a Default graph, to which is an implicit argument to all API
functions. So tf.tensors and tf.operatioins are automatically added to this graph.

fuer einen beispielhaften tf.Graph abbgebildet. Daneben befindet sich eine
Visualierung des erstellen Graphs.
...
\paragraph{Tensoren}
Jeder Knoten in TF ist ein Tensor, wie wir sie bereits in Sektion () einmal kennengelernt
haben. Tensoren werden deshalb als Datentraeger in TF verwendet,
da die Daten so beliebige Dimensionalitaten und Formen annehmen koennen.
Es koennen sowohl Listen von Zahlen (Vektoren), wie auch Bilder (Tensoren
dritter Ordnung) verrechnet werden. \\
In TensorFlow bezeichnet man einen Tensor als einen \keycode{tf.Tensor}. Er
ist definiert durch seine Form (mit Stufe) und den Datentyp seiner Elemente (z.B. float32).

Spezielle tf.Tensoren:
\begin{itemize}
\item{tf.Variable}
\item{tf.constant}
\item{tf.placeholder}
...
Alle TEsnoren ausser tf.variable sind immutable, also besitzen einen einzigen
Wert pro Ausfuehrung des Graphs.
...
After the graph has been launched in a session, the value of the Tensor can be computed by passing it to tf.Session.run. t.eval() is a shortcut for calling tf.compat.v1.getdefaultsession().run(t).
]

Einen Tensor definiert man folgendermassen (in einem tf.Graphs):
...
als \keycode{tf.Operation}. Eine tf.Operation besitzt jeweils Namen und reprasentiert eine
abstrakte Berechnung. Zum Beispiel eine Matrix-Multiplikation (in TF als
``MatMul'' bezeichnet),
eine Addition (``Add'') oder eine Aktivierungsfunktion, wie die ReLU (``ReLU'').
Zum Teil besitzen Operationen noch gewisse Attribute, welche sie genauer
beschreien, z.B. die Anzahl Filter bei der Anwendung einer Convolutional-Schicht.

[An Operation is a node in a TensorFlow Graph that takes zero or more Tensor objects as input, and produces zero or more Tensor objects as output. Objects of type Operation are created by calling a Python op constructor (such as tf.matmul) or tf.Graph.createop.
...
After the graph has been launched in a session, an Operation can be executed by passing it to tf.Session.run. op.run() is a shortcut for calling tf.compat.v1.getdefaultsession().run(op).]

Die einfachste Operation ist eine tf.constant(), welche einfach einen konstanten
Wert zurueckgibt. a = tf.constant(3.0, dtype=tf.float32)

Code zur Definition eines Operation
...
zu erstellen und ihn auszufuehren. Sie
...
Code zur Defintion einer Session
...
If you want to change the shape of a variable later you have to use an assign Op with validateshape=False.
...
\paragraph{Feeding}
\keycode{tf.Placeholder} sind Parameter fuer Computation Graph
...
\begin{minted}[frame=lines,framesep=2mm,baselinestretch=1.2,bgcolor=lightgray,fontsize=\footnotesize,linenos]{python}
  ...
  sess.run(init) // initalizing all veriables
\end{minted}

\subsubsection{Training}
Forwardpropagation durch Graph und Backpropagation speziell implementiert..
...
\item{Distributed execution. By using explicit edges to represent the values that flow between operations, it is possible for TensorFlow to partition your program across multiple devices (CPUs, GPUs, and TPUs) attached to different machines. TensorFlow inserts the necessary communication and coordination between devices.}
...
leicht Paralellisieren lassen. Diese lassen sich leicht Parallelisieren, da wir
es mit Dataflow-programming zu tun haben. Deshalb werden gerichtete Graphen verwednet.
...
\subsection{Keras-Schichten}

\subsubsection{Conv2D}
\subsubsection{MaxPooling2D}
\subsubsection{UpSampling2D}
