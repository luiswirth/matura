...
Element, der sogennante \keyword{Quellenpixel} (engl.: Sourcepixel), bezeichnet mit $(\ten{F})_S$,
...
des Quellenpixel $\mat{F}_Q$ im rezeptiven Feld angiebt. Somit ist das rezeptiven Feld einfach eine
Untermatrix $\tilde{\mat{B}} \in \set{R}^{3 \times 3}$ des Obermatrix $\mat{B}$.

\begin{figure}[h!]

  % Quellenpixel einzeichnen mit Beschriftung
...
rezeptives Feld haben. Dadurch kommt das Quellenpixel nie auf den Raendern des
...
Zur Erinnerung: Dies liegt daran, dass pro Lage des Quellenpixel nur ein Pixel
des neuen Bildes entsteht und das Quellenpixel nunmal nicht auf allen
...
deutlich kleiner geworden ist, denn das Quellenpixel uebspringt somit auch
...
\begin{tikzpicture}[every node/.style={anchor=north east,fill=white,minimum width=1.4cm,minimum height=7mm}]
  ...
  \matrix (mA) [matrix of math nodes,left delimiter={(},right delimiter={)}]
  {
    w_{i\,|\,1,1,c^l}^l & \cdots & w_{i\,|\,1,f,c^l}^l \\
    \vdots & \ddots & \vdots \\
    w_{i\,|\,f,1,c^l}^l & \cdots & w_{i\,|\,f,f,c^l}^l \\
...
\begin{tikzpicture}[every node/.style={anchor=north east,fill=white,minimum width=1.4cm,minimum height=7mm,inner sep=0,outer sep=0}]
  ...
  \node (mB) at ($(mA.south west)+(0.25,0.125)$)
  {
    $\begin{bmatrix}
      w_{i\,|\,1,1,\gamma}^l & \cdots & w_{i\,|\,1,f,\gamma}^l \\
      \vdots & \ddots & \vdots \\
  ...
  \node (mC) at ($(mB.south west)+(0.25,0.125)$)
  {
    $\begin{bmatrix}
      w_{i\,|\,1,1,1}^l & \cdots & w_{i\,|\,1,f,1}^l \\
      \vdots & \ddots & \vdots \\
  ...
  \draw[dashed,gray](mA.north east)--(mC.north east);
  \draw[dashed,gray](mA.north west)--(mC.north west);
  \draw[dashed,gray](mA.south east)--(mC.south east);
\end{tikzpicture}
