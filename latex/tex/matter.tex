...
$\ten{A}$ an und erhaelt so ein neues Bild $\ten{A}^*$. Man schreibt dafuer:
$\ten{A}^* = \ten{W} * \ten{A}$. \\
...
Wir beziechnen dieses Feld mit $\tilde{\mat{A}}^{(y,x)}$, wobei die hochgestellten Indizes $(y,x)$ die Position
des Zentralelements im rezeptiven Felde angiebt. Somit ist das rezeptiven Feld einfach eine
...
gespiegelt wird, im Gegensatz zu der eigentlichen Faltung. Begrifflich benutzt
man jedoch die Woerter Faltung und Convolution um beide
...
folgende Formel verwenden um einen Pixel $\mat{A}^*(x,y)$ des neuen Bildes zu berechnen.
\\
\begin{equation}\tag{CO1}
...
den Filter gerade kompensiert. Da durch das Padding der Filter der Laenge und
...
jede Faltung einen neue Matrix $\mat{B}^*_i$ liefert, wobei $i$ der Index des
