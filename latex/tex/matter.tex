...
essentiell, um in Sektion \refbox{sec:convolutional_autoencoder} die Topologie
...
zu entwickeln. Desweiteren sind die performanten
Implementierungen bemerkenswert, welche den Trainingsvorgang sehr kurz halten.
Daher ist TF gegenüber selbstgeschriebenen Implementierungen zu bevorzugen.
...
Trotz des engen Bezugs zum Maschinellem Lernen, eignet sich TF darüber hinaus auch für
anderstartige Anwendungen, welche Gebrauch von numerischen Berechnungen machen.
\para{}
...
\footnote{Momentan befindet sich TF 2.0 in der Beta Version. Es handelt sich
  dabei um eine grosse Überarbeitung des Frameworks. Sobald TF 2.0 offiziel
...
\begin{infobox}{Fontend und Backend}
  ...
  ohne die zugrundeliegenden Logik preiszugeben.
  \para{}
...
Es handelt sich dabei um einen \keyword{Computatial Graph}, welcher schon in
...
abgebildet. Die gewählte Abbildung wird als \keycode{Operation} bezeichnet.
...
\para{}
TensorFlow offeriert eine Vielzahl von vordefinierten Operationen, so dass eine eigenständige
Definition von Operationen meistens nicht notwendig ist.
Ein Beispiel für eine vordefinierte Operationen ist die
...
Im Rahmen dieser Arbeit wurde jedoch nur das SGD-Verfahren behandelt, so dass
...
bezüglich der Modellparametern bestimmen. Hierfür macht sich die Verwendung von
...
Dies ist ein Modell, welches einen linearer Stapel von verschiedenen
...
Die Defintion des Graphen in Keras erfolgt, indem die verschiedenen Schichten
...
zur Initalisierung der Modellparameter verwendet, als in der Theorie (siehe
Sektion \refbox{sec:parameter_initalisieren}) erklärt wurde.
...
Glorot-Initalisierung mit Normalverteilung angewendet werden soll.
\para{}
...
überschreiben wir den Standard-Initalisierer mit dem
\code{'glorot\_normal'}-Initalisierer. Ebenfalls wird die vorherige Schicht
...
zusammengefasst werden soll. Desweiteren müssen der Stride $s$ und das Padding $p$
spezifiziert werden.
...
\keycode{UpSampling2D}. Zur Definiton muss die Grösse $f$ des neuen Feldes
...
% #TODO: Ausführlichere Modellbeschreibung mit Anzahl Schichten etc. -> erlaubt
% Kürzung in Programmierabschnitt

\begin{figure}[h!]
  \centering
  \includegraphics[width=0.9\textwidth]{themodel.pdf}
