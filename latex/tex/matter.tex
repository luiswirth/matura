...
$\ten{B}$ an und erhaelt so ein neues Bild $\ten{B}^*$. Man schreibt dafuer:
$\ten{B}^* = \ten{W} * \ten{B}$.
\para{}
...
folgende Formel verwenden um einen Pixel $\mat{B}^*(x,y)$ des neuen Bildes zu berechnen.
\\
\begin{equation}
  \mat{B}^*_{y,x} = (\mat{W} * \mat{B})_{y,x} = \sum_{v=0}^{f-1} \sum_{u=0}^{f-1} \mat{W}_{v,u}\mat{B}_{y+v,x+u}
\end{equation}
\\
...
Bild $\mat{B}^*$ immer eine 2D-Matrix ist.
Die allgemeine Filtergleichung lautet:
\\
\begin{equation}\tag{FO}
  \mat{B}^*_{y,x} = (\ten{W} * \ten{B})_{y,x} = \sum_{v=0}^{f-1} \sum_{u=0}^{f-1} \sum_{w=0}^c \ten{W}_{v,u,w} \ten{B}_{x+u,y+v,w}
\end{equation}
...
jede Faltung einen neue Matrix $\mat{B}^*_i = \ten{F}_i * \ten{B}$ liefert, wobei $i$ der Index des
...
diese Art von Features als \keyword{lokalisierte Features}.
\para{}
...
Somit nehmen die Bilder
die Rolle der Aktivierungen $\vec{a}^l$ ein, weshalb man sie mit $\ten{A}^l$ bezeichnet.
Das Bild $\ten{B}^0$ ist hierbei wieder der Input $\ten{X}$ ins ganze Netz.

Da wir nun die Filter in ihrem vollen Umfang betrachtet haben, koennen wir uns
endlich den Convolutional Schichten widmen.
Eine ConvSchicht $l$ erhaelt als Input ein Bild $\ten{B}^{l-1} \in \set{R}^{h^{l-1}
...
werden aufeinander draufgelegt und bilden den Tensor $\ten{B}^{*,l} \in \set{R}^{h^l
  \times w^l \times c^l}$.
...
Aktiverungstensor $\ten{B}^l \in \set{R}^{h^l \times w^l \times c^l}$.
