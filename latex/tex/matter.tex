...
später erläuterte Autoencoder streng genommen (aufgrund einer irreführender Zuordnung) zum Unüberwachten Lernen, auch wenn
es der gleichen Systematik des Überwachten Lernens folgt (siehe Sektion~\refbox{sec:autoencoder}).
\para{}
GUTE QUELLE (Pionier)
...
Unter korrekten Bedingungen (kein Overfitting (siehe \refbox{sec:overfitting})) findet kein Auswendig lernen der Trainingsdaten statt,
...
$2$ weg fällt. Sie hat keine entsprechende Verlustfunktion, da man so das
...
\refbox{sec:parameter_initalisieren} erläutert.
\para{}
...
Der Gradienten eines genug grossen Mini-Batches ist zwar nicht ganz exakt, aber approximiert den Gradienten des gesamten Datensatzes genügend gut.
...
Der Grund dafür soll später in Sektion \refbox{sec:künstlicheNeuronen} erläutert werden.
\para{}
...
Sie hat einen einzigen Stufensprung bei $x=0$ vom Wert 0 auf 1 (siehe Abb. \refbox{fig:heaviside}).
\\
\begin{figure}[h!]
  \begin{minipage}[h!]{0.5\textwidth}
    \begin{equation*}
...
auf sich selbst linear abgebildet und die Werte kleiner als 0 werden auf 0
...
Kapitel \refbox{sec:CNN} im Zusammenhang mit KNNs zur Bilderkennung wieder betrachten.
\para{}
\begin{figure}[h!]
  \begin{minipage}[h!]{0.5\textwidth}
    \begin{equation}
...
\refbox{fig:sigmoid}). Desweiteren zeichnet sie sich durch eine vergleichsweise
...
Abbildungen \refbox{fig:nomenklatur1}) und (\ref{fig:nomenklatur2} zum
...
ein Vielfaches. Später in Sektion \refbox{sec:tensorflow} wird dies nochmals thematisiert.
\para{}
...
In der Konsequenz verlangsamt sich der Lernprozess ohne nahe genug am lokalen
Minimum zu sein. Dies wird auch als Saturierung der Neuronen bezeichnet (FUSSNOTE?).
\para{}
In der Konsequenz wurde eine neue Technik entwickelt: die Glorot-Initialisierung
...
andere Möglichkeit besteht in der Initialisierung der Neigungen mit 0, wie Keras
das etwa auch macht.
\begin{equation}
  b_{t=0}^l = 0
\end{equation}
...
dass das SGD für KNNs einen eigenen Namen besitzt: die sogenannte
...
\refbox{sec:anhang_bp} zu verweisen, in welchem auf das Konzept eines
...
Pixel gewählt (dies entspricht weniger als $0.07$ Megapixel, im Vergleich dazu, hat ein iPhone XS hat eine Kamera mit
...
\refbox{fig:filter2d}) und Abb. (\ref{fig:filter3d}). Solche Filter sind immer quadratisch, wobei ihre
...
\para{}
Nun wird ein beispielhafter Kantendetektionfilter betrachtet. Er besitzt die
...
In Abbildung \refbox{fig:sobel_filter} wurde der erwähnte Kantendetektionfilter auf ein
...
Bilder erklärt (siehe Sektion \refbox{sec:filteroperation_mathematisch}).
\para{}
...
\keyword{Rezeptives Feld} des Filters (siehe Abb. \refbox{fig:receptive_field}).
...
von dem Problem mit der grossen Menge an Modellparametern, siehe Sektion \refbox{sec:CNN_parameter_problem})
\para{}
...
\begin{figure}[h!]
  \centering
  \includegraphics[width=0.9\textwidth]{cdae.pdf}
...
eigentlichen Implemention von TF (dem Master) kann er im Anhang \refbox{sec:anhang_tf} dazu
...
Es handelt sich dabei um einen \keyword{Computation Graph}, welcher schon in
Anhang \refbox{sec:anhang_bp} erwähnt wurde.
In TensorFlow wird dieser als einen \keycode{Graph} bezeichnet.
\para{}
...
\begin{figure}[h!]
  \caption{Visualisierung eines \code{Graph}}
\end{figure}

\subsubsection{Knoten und Tensoren}
Die zentrale Dateneinheit in TensorFlow sind Tensoren, welche bereits in Sektion
\refbox{sec:tensor} präsentiert wurden. Auch die Knoten des Graphen
...
\refbox{sec:forward}) und im Anhang (\ref{sec:anhang_bp} die Gleichungen in
Matrixform herzuleiten. \para{} In Tensorflow bezeichnet man einen Tensor mit
\keycode{Tensor}. Er ist definiert durch seine Form und den Datentyp seiner
...
(\keycode{placeholder}). Eine andere Art wird verwendet um die Modellparameter
einzuführen (\keycode{variable}). Die Modellparameter-Tensoren sind im Gegensatz
...
als \keycode{Operation}. Eine Operation besitzt jeweils einen Namen und repräsentiert eine
...
Ein Beispiel dafür wäre die \keycode{matmul(a,b)}-Operation, welche eine
Matrixmultiplikation zwischen dem Knoten \mintinline{python}{a} und dem Knoten \mintinline{python}{b}
ausführt. Das Resultat kann dann in einer neuen Knotenvariable
\mintinline{python}{c} gespeichert werden.
\para{}
...
\keycode{Session} zuständig. Sie schickt den Graphen an das Backend, damit er
...
muss es trainiert werden. Dafür verwendet man einen sogennanten \keycode{Optimizer}.
...
Dieses Vorgehen wird in Sektion \refbox{sec:anhang_bp} ausführlich erklärt.
...
Damit dies nicht nötig ist, verschaffen wir uns Abhilfe mit der High-Level API \keyword{Keras}.
...
\refbox{sec:parameter_initalisieren} erklärt.
...
überschreiben wir den Standard-Initalisierer mit dem \code{'glorot\_normal'}-Initalisierer.
\begin{minted}[frame=lines,framesep=2mm,baselinestretch=1.2,bgcolor=lightgray,fontsize=\footnotesize,linenos]{python}
...
\keycode{MaxPool2D}. Um sie zu definieren, muss die zusammenfassende Feldgrösse
...
\keycode{UpSampling2D}. Dabei muss die Feldgrösse $f$ und die
...
Dies macht man mit der Methode \code{compile} diese erwartet als
...
Das Ergebniss ist in Abbildung \refbox{fig:noisy_clean_mnist} sichtbar.
\begin{figure}[h!]
  \centering
...
wie in Sektion \refbox{sec:parameter_initalisieren} statt. \\
...
\end{table}
\para{}
Nun wird das Modell mit den optimalen Hyperparametern
nochmals für 100 Epochen trainiert, um danach nochmals Bilder zu entrauschen.
Diesmal stellen wir den \code{noise\_factor} bei der Generierung des Rauschen
auf 1. Die Ergebnisse sind in Abbildung zu sehen.
\begin{figure}[h!]
  \centering
  \includegraphics[width=0.9\textwidth]{denoised.png}
  \caption{Verrauschte Bilder, Rekonstruktionen und Orignalbilder}
...
Grundsätzlich ist das Modell ziemlich gut. Es ist in der Lage ein stark
verrauschtes Bild, welches für einen Menschen schwer entzifferbar ist,
vollständig von seinem Rauschen zu befreien.

\subsubsection{Hyperparameter Ergebnisse}
...
einhergeht. Somit ist der Gradientenabstieg exakter. Jedoch existiert eine
entsprechende Schattenseite: Die Motivation hinter der Einführung der
Minibatches für SGD liegt darin die Trainingszeit zu verkürzen. Durch die
kleineren Batches jedoch die Trainingszeit erheblich zu.
Eine weiteres Ergebniss ist, dass eine erhöhte Filteranzahl das Modell bessere
Resultate erbringen lässt. Jedoch ist der Einfluss dieses Hyperparameter am
geringsten, von allen die hier getestet wurden.
\para{}
...
der Entwicklung des Denoisers. Diese hätten wohlmöglich für noch bessere
Resultate gesorgt. Aufgrund der Länge der Arbeit, wurde jedoch darauf
verzichtet.
\para{}
Die erste Verbesserungsmöglichkeiten bestünde darin, dass noch mehr und
ausführlicher die Hyperparametern auszutesten. Die Topologie und die
Schichtenarten wurden beispielsweise gar nicht verändert. Dies hat den
praktischen Grund, dass die Anzahl Permutationen an Hyperparameter nohcmals
dermassen gestiegen wäre, dass das Trainings kein Ende gefunden hätte.
Ausserdem ist es schwierig bei einem Convolutional-Autoencoder verschiedene
Topologien automatisiert zu testen, denn es muss garantiert werden, dass die
Inputschicht die gleichen Dimensionen wie die Outputschicht besizt. Dies stellt
sich als Herausforderung dar.
\para{}
Eine weitere Modifikation, welche das Modell vermutlich deutlich verbessert hätte,
wäre eine alternative Kostenfunktion gewesen. Beispielsweise die
Kreuzentropy-Kostenfunktion, welche deutlich raffinierter ist als die MSE-Kostenfunktion.
Sie stellt eine Lösung für das sogennante \keyword{Vanishing-Gradient-Problem}
dar. Durch dessen Behebung, kommt es nicht zu einer verlansam der Trainings, im
Verlauf des Gradientenabstiegs.
\para{}
Weitere Verbessungen könnten in der Wahl eines anderen
Optimisierungsverfahren liegen. Beispielsweise könnte eine SGD-Variante mit
Impuls (engl.: momentum) verwendet werden. Diese Art von SGD stellt ein
Verfahren dar, welches den Gradientenabstieg weniger sprunghaft macht und damit beschleunigt.
\para{}
Viele dieser Modifikationen wären programmatisch in TensorFlow nur wenige
Codeänderungen entfernt gewesen. Jedoch ohne zugrundeliegende theoretische
Behandlung der Funktionsweise, hält sich der Wert der Verwendung dieser Methoden
in Grenzen.
...
% LocalWords:  Gymnasialstufe Gradientenverfahrens parameter initalisieren
