...
auf der Ebene der Netzform. Autoencoder werden verwendet um \keyword{effiziente
  Daten-Codierungen} zu erlernen und kann so zur
\keyword{Dimensionalitaetsreduktion} verwendet werden.
Die Aufgabe eines Autoencoders besteht darin eine Repraesentation einer Datenmenge
zu erlernen. Diese Repraesentation soll mit weniger Daten die fast gleiche
...
beizubehalten. \\
Neben dieser neuen Repraesentation erlernt das Netzwerk nebenbei noch, wie es
...
sie dem unueberwachtem Lernen an, da ihre Daten keine Labels beinhalten. Jedoch
ist diese Begrifflichkeit etwas irrefuehrend, da der Autoencoder trotzdem darauf
trainiert wird, gewisse gewuenschte Outputs zu liefern. Die Labels sind naemlich gerade
gleich den Inputs. Also versucht ein Autoencoder mit seinen Vorhersagen einfach
die Features so exakt wie moeglich zu reproduzieren.
...
ziemlich bedeutungslos und ziemlich trivial ausfuehrbar. Aus diesem Grund muss
man gewisse Einschraenkungen einfuehren, welche dem Netzwerk nur eine
approximative Rekonstruktion erlauben.
\para{}
Die Einschraekung besteht darin, dass dem Autoencoder nicht beliebig viele
Variabeln zur Codierung zur Verfuegung stehen. Das Modell muss sich auf nur
wenige Zahlenwerte beschraenken um die Features zu beschreiben. Deshalb wir das
Netz dazu gezwungen eine neue Repraesentation, das sogennante \keyword{Encoding},
zu erlernen. Das Encoding umfaesst lediglich noch die wichtigsten
Merkmale der Daten, welche die ausdurckstaerksten Eigenschaften einfaegt. Somit
hat das Encoding eine niedrigere Dimensionalitaet als die Inputs. Ein Datensatz
...
Encoding die urspruengliche Repraesentation zu rekonsturieren. Diese
...
$(f,g)$, ueber einen $d$-dimensionalen Eingaberaum $\fspace{X} \subseteq \set{R}^d$. Die
Elemente des Eingaberaums sind einfach die Features $\vec{x} \in \fspace{X}$.
Neben dem Eingaberaum existiert noch ein ein $p$-dimensionaler Encodingraum
$\fspace{E} \subseteq \set{R}^p$. Dabei ist $p \ll d$, wodurch er eine
niedrige-dimensionale Coderiung darstellt.
Gesucht werden nun die Funktionen
\begin{itemize}
\item{$f: \fspace{X} \to \fspace{E}$ (Encoder-Funktion)}
\item{$g: \fspace{F} \to \fspace{X}$ (Decoder-Funktion)}
\end{itemize}
...
\begin{equation}
  |\vec{x} - (g \circ f)(\vec{x})| \to \text{min}
\end{equation}
...
Die Einschraenkungen welche das Netz zur Komprimierung der Daten zwingt werden
durch die Topologie des Netzes realsiert. Die Netzform macht den Charakter des
Autoencoder aus. Diese ist naemlich

Einfachster Autoencoder ist ein KNN mit drei Schichten:

\begin{itemize}
\item{Eingabeschicht mit $d$ Eingabeneuronen}
\item{Zwischenschicht mit $p$ Neuronen, bezeichnet als \keyword{Flaschenhals}}
\item{Ausgabeschicht mit $d$ Ausgabeneuronen}
\end{itemize}
Der Trainingsdatensatz ist der Eingaberaum $X$.

Man kann einen Autencoder auch aus mehr Zwischenschichten bauen und dabei jede
Schicht zum Falschenhals hin kleiner machen und die Schichten die davon
wegverlaufen wieder anwachsen lassen.
