...
Element, das sogennante \keyword{Zentralelement}, bezeichnet mit $(\ten{F})_z$,
...
Wir bezeichnen dieses Feld mit $\tilde{\mat{B}}^{(y,x)}$, wobei die hochgestellten Indizes $(y,x)$ die Position
des Zentralelements $\mat{F}_z$ im rezeptiven Feld angiebt. Somit ist das rezeptiven Feld einfach eine
...
hat der Filter zu beginn das rezeptive Feld $\tilde{\mat{B}}^{(2,2)}$, denn der
...
Weshalb verwendet man als nicht einfach KNNs (abgesehen
von dem Problem mit der Ueberzahl von Modellparametern, siehe Sektion \ref{sec:CNN_parameter_problem}).
\para{}
...
Kontraste voneinander trennt. Oder ein Gegenstand wird erst durch eine ganzen Haufen
von Pixeln und deren relative Position untereinander ausgezeichnet.
Man bezeichnet diesen Umstand als \keyword{lokalisierte Features}.
\para{}
Desweiteren sind Bildausschnitte nicht immer gleich
orientiert sind. Wenn man ein Gesicht auf einem Bild erkennen moechte, sollte es aber
keine Rolle spielen, wo auf dem Bild es sich befindet, welche Groesse es
hat und was fuer eine Lage es hat. Um diese Eigenschaften zu ignorieren, muss
das Modell gewisse \keyword{Invarianzen} erfuellen.
\para{}
Die Eigenschaft, welche CNN dafuer geeignet macht fuer lokalisierte Features und
solche Invariazen macht, bezeichnet man als \keyword{Parameter-Sharing}.
Sie bezeichnet den Umstand, dass auf
mehrere oder alle Features die gleichen Modellparameter angewandt werden, durch
die Bewgung der Filters und seinen Eintraegen ueber (fast) alle Pixel. Somit ist
es egal wo sich die lokalisierten Features befinden. Dies fuehrt dann eben zu
den gewuenschten Varianzen: Translationsinvarianz, Rotationsinvarianz und Helligkeitsinvarianz.
\para{}
Noch weitere Vorteile eines CNNs bestehten darin, dass sie beliebige Groessen
von Inputbildern verarbeiten koennen, da die Modellparameter die Features gleich
behandeln.

\section{die Convolutional-Schicht}
Nun moechten wir nochmals alles zusammentragen, was wir ueber Filter gelernt
...
\end{equation*}
\begin{equation*}
  (\ten{W^l}_i)_{:,:,\gamma} = \begin{pmatrix}
    w_{i\,|\,1,1,\gamma}^l & \cdots & w_{i\,|\,1,f,\gamma}^l \\
    \vdots & \ddots & \vdots \\
    w_{i\,|\,f,1,\gamma}^l & \cdots & w_{i\,|\,f,f,\gamma}^l
  \end{pmatrix}
\end{equation*}
\begin{equation*}
  (\ten{W^l}_i)_{:,:,c^l} = \begin{pmatrix}
...
\end{equation}
\\
Jede dieser Matrizen ist eine Tiefenschicht $\tilde{\mat{A}}^l_{:,:,i}$ des neuen
3D-Tensors $\tilde{\mat{A}}^l \in \set{R}^{h^{l+1} \times w^{l+1} \times c^l}$.
...
\end{equation}
\\
Nun wendet man in einem letzten Schritt die vektorisierte Aktivierungsfunktion
$\vecf{\varphi}$ auf diesen Tensor $\tilde{\mat{A}}^l$ an und erhaelt so die
...
\section{Upsampling-Schichten}

\section{Fully-connected-Schichten}
...
\section{VAEs als generative Modelle}
Erwaehnung von GANs
