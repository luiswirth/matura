...
Maschinellem Lernen zusammen mit dem konkreten Modell eines KNN und deren
konkreten Architekturen des CNN und des Autoencoder dargelegt.
...
liegt. TF zeichnet sich hauptsächlich durch seine einfache Anwendung aus, welche
es erlaubt ohne viele mathematische Vorkenntnisse ein ML-Modell zu definieren,
...
definieren und mit en eingebauten Algorithmen zu trainieren.
...
wird. Somit wird nicht direkt mit TF gearbeitet, trotzdem ist es hilfreich ein
...
der Konsequenz jedoch ist der deutlich anspruchsvoller und ausführlicher (engl.:
verbose).
Da der Benutzer jedoch keine direkte Interaktion mit dem Master hat, muss er
sich nicht mit C++ befassen.

\subsection{Verwendung --- der Client}
Nun wird erörtert, wie TensorFlow verwendet werden kann, um ein Modell zu
definieren und zu trainieren. Es wird als der Client betrachtet. Mithilfe diesem Wissen kann nachvollzogen werden,
wie Keras als Schnittstelle, es dem Benutzer erleichtert ein Modell zu erstellen.
Dieser Abschnitt und die folgenden setzen gewisse Python-Kenntnisse voraus.
\para{}
TensorFlow bitet verschiedene Konzepte, um programmatisch zu arbeiten.
Im folgenden Abschnitt werden wir diese Konzepte behandeln.
...
aufgestellt und fügt ihm Knoten hinzu. Erst nach der Definition wird er dann ausgeführt.
Es handelt es sich dabei um einen \keyword{Computation Graph}, welcher schon in
Anhang (\ref{sec:anhang_bp}) erwähnt wurde.
In TensorFlow wird dieser als als einen \keycode{Graph} bezeichnet.
\para{}
...
\para{}
Man muss einen Graph nicht explizit definieren, da beim Definieren der
Operationen, sie implizit dem sogenannten Default Graph hinzugefügt
werden.
...
\subsubsection{Tensoren und Knoten}
Die zentrale Dateneinheit in TensorFlow sind Tensoren, welche bereits in
Sektion (\ref{sec:tensor}) präsentiert wurden.
Auch die Knoten des Graphen repräsentieren Tensoren. Die meisten jedoch
enthalten keine festen Zahlenwerte, sondern werden erst bei der
der Ausführung des Graphen befüllt. Danach können die Werte ausgelesen werden.
\para{}
Die Tensoren erlauben es den Daten innerhalb des Graphen belibige
Dimensionaltäten und Formen anzunehmen. Es können sowohl Listen von Zahlen
(Vektoren), wie auch Bilder (Tensoren dritter Stufe) verrechnet werden.
Aus diesem Grund war es auch sinnvoll in Sektion (\ref{sec:forward}) und im
Anhang (\ref{sec:anhang_bp}) die Gleichungen in Matrixform herzuleiten.
\para{}
In Tensorflow bezeichnet man einen Tensor mit \keycode{Tensor}.
Er ist definiert durch seine Form (z.B. (2,2,2)) und den Datentyp seiner
Elemente (z.B. tf.float32).
\para{}
Fast alle Tensoren ist mit einer einzigen Graph-Ausführung assoziert. Sie
...
\para{}
Neben dieser Art von Knoten, gibt es auch noch Inputknoten.
Es gibt verschiedene von ihnen und jeder stellt eine Art von Input in den
Graphen dar. Daher müssen sie explizit definiert werden.
Die verschiedene Inputknoten werden wir nun betrachten.

\paragraph{constant}
Der \keycode{constant}-Tensor ist ein Input-Knoten in den Graphen.
Er hat immer einen konstanten Wert, welcher beim Erstellen
angegeben wird. Seine Definition hat folgenden Charakter:
\begin{minted}[frame=lines,framesep=2mm,baselinestretch=1.2,bgcolor=lightgray,fontsize=\footnotesize]{python}
  <Variable> = tf.constant(<Elemente>)
\end{minted}
\para{}
...
\end{minted}
\[\text{\code{skalar}}=1 \quad \text{\code{vector2}}=\begin{pmatrix}1&2\end{pmatrix} \quad
  \text{\code{matrix2x2}}=\begin{pmatrix}1&2\\3&4\end{pmatrix}\]

\paragraph{placeholder}
Um den Graphen für Maschinelles Lernen zu verwenden, muss er mit Modellinputs
befüllt werden können.
Dafür eignet sich der \keycode{placeholder}-Tensor. Wie es der Name schon sagt,
ist er ein Platzhalter für die Inputs, welche beim Start der Graphausführung
eingespeist werden. Bei der Erstellung muss einzig seine Form und sein Datentyp
angegeben werden.
\para{}
\begin{minted}[frame=lines,framesep=2mm,baselinestretch=1.2,bgcolor=lightgray,fontsize=\footnotesize]{python}
  <Variable> = tf.placeholder(dtype=<Datentyp>, shape=<Form>)
\end{minted}
Für den Datentyp wählt man eigentlich immer
\mintinline{python}{dtype=tf.float32}, da sich Gleitkommazahlen am besten für ML eignen.
...
\paragraph{variable}
Eine weitere Vorausetzung um Maschinelles Lernen mit dem Graphen durchzuführen
ist, dass Modellparameter eingeführt werden können. Dafür ist der
\keycode{variable}-Tensor geeignet. Er ist im Gegensatz zu den anderen
Tensoren ``mutable''. Das bedeutet, dass seine Werte unabhängig von einer
einzigen Graphausführung existieren und erhalten bleiben. Seine Elementen
können können von Ausführung zu Ausführung modifiziert werden. Somit sind sie
geeignet um Modellparameter einzuführen.
Die Modifizierung der Werte findet generell nur im Trainingsdurchlauf statt.
\para{}
Eine Variable definiert man folgendermassen, mit einem Initalwert.
Folgender Python-Code kreirt eine tf.Variable mit einem Anfangswert und ändert ihren Wert danach.
\begin{minted}[frame=lines,framesep=2mm,baselinestretch=1.2,bgcolor=lightgray,fontsize=\footnotesize]{python}
  <Varname> = tf.Variable(<Initialwert>)
