...
kann das Modell Vorhersagen für neün Daten machen.
\para{}
...
Nun werden iterativ neü Punkte $\vec{p}_t$ gesucht, welche immer näher beim lokalen Minimun liegen, also Punkte, die den Funktionswert $f(\vec{p}_t)$ immer kleiner werden lassen.
Dies wird durchgeführt, bis der Punkt genügend nahe beim lokalen Minimun ist.
\para{}
Dafür muss ein Vektor $\vec{b}_t$ bestimmt werden, welcher auf den Punkt $\vec{p}_t$ addiert einen neün Punkt $\vec{p}_{t+1}$ bildet,
...
\para{}
\cite{wiki:künstliches_neuronales_netz}
...
> 0$. Der neü Term $\tilde{z} + b$ wird mit $z$ bezeichnet (siehe Rest der Gl. (\ref{eq:perzeptron_1})).
...
Hierfür muss aber noch ein neüs mathematisches
...
\begin{defbox}{Vektorisierung einer Funktion}
  ...
  Komponenten jeweils \textit{einzeln} die Funktion $f$ angewandt wird. Dieser neü
...
muss eine neü Operation eingeführt werden: das Hadamard-Produkt.
...
$\ten{B}$, welche
über alle Pixel wandert, diese manipuliert und so wieder ein neüs Bild
$\tilde{\ten{B}}$ liefert.
...
Angewandt auf ein schwarz-weisses Bild, erhält man ein neüs Bild, bei welchem alle erkannten
...
$\ten{B}$ an und erhält so ein neüs Bild $\tilde{\ten{B}}$. Man schreibt dafür:
$\tilde{\ten{B}} = \ten{W} * \ten{B}$.
\para{}
...
gebildet. Nun erhalten wir eine neü Matrix $\mat{W} \odot
\hat{\mat{B}}^{(2,2)} = \mat{P} \in \set{R}^{3 \times 3}$.
Nun werden noch alle Elemente der neün Matrix $\mat{P}$
...
bestimmt, wie die neün Pixel aussehen.
\\
...
\begin{figure}[h!]
  ...
  \begin{tikzpicture}[x={(0.866cm,0.5cm)}, y={(-0.866cm,0.5cm)}, z={(0cm,1cm)}, scale=0.7,every pin/.style = {pin edge={Latex-,thin,black}}]
    ...
    \node [pin=180:Qüllenpixel $(\mat{B})_S$] at (n1-2-2.east) {};
    \begin{scope}[canvas is xz plane at y=0,transform shape]
      \node[draw=red,minimum size=1cm] (marked1) at (2,-2) {\LARGE 2};
      % \node[] (sp-tag) at (n-1-2-2.north) {Qüllenpixel $(\mat{B})_S$};
    \end{scope}
...
Es ist zu bemerken, dass das neü Bild $\tilde{\mat{B}} \in \set{R}^{(h-2) \times (w-2)}$ nicht mehr die gleichen Masse
hat, wie das Ursprungsbild $\mat{B} \in \set{R}^{h \times w}$. Das liegt daran, dass pro
Lage des Filters jeweils nur ein Pixel des neün Bildes berechnet wird. Man kann
sich vorstellen, dass an der Position des Qüllenpixel jeweils ein neür
...
folgende Formel verwenden, um einen Pixel $\tilde{\mat{B}}(x,y)$ des neün Bildes zu berechnen.
\\
\begin{equation}
...
jede Faltung einen neü Matrix $\tilde{\mat{B}}_i = \ten{W}_i * \ten{B}$ liefert, wobei $i$ der Index des
...
\end{figure}
Die Tiefe des neün Bildes $\tilde{\ten{B}}$ ist jetzt gerade die Anzahl Filter $c$.
...
des neün Bildes entsteht und das Zentralelement nunmal nicht auf allen
...
neü Grösse $n_1$ des Bildes, anhand der alten Grösse $n_0$. $n$ bezeichnet
...
Tensor wird der Länge und der Breite nach (nicht der Tiefe entlang!) an den Enden erweitert. Die neün Elemente werden dabei auf den Wert
$0$ gesetzt.
...
Position übersprungen. Zur Folge hat dies, dass das neü Bild
...
\para{}
Folgende Formel beschreibt die Dimensionen des neün Bildes unter
...
$\ten{A}^l$ und erhält so mehrere neü 2D-Bilder $\tilde{\mat{A}}_i^l$.
\\
\begin{equation}
...
Tiefe des neün 3D-Tensors $\tilde{\ten{A}}^l \in \set{R}^{h^{l+1} \times w^{l+1} \times c^l}$.
...
neü Aktiverung der nächsten Schicht $\ten{A}^{l+1}$. Empirisch hat man
...
\para{}
Um zu berechnen, was die neün Dimensionen nach dem Pooling sind, gelten die
...
Dimensionalität.
\para{}
Es stellt sich natürlich die Frage, was für Werte das neü Feld erhält.
...
ziemlich trivial, denn alle neün Feldelemente nehmen einfach den Wert des alten
...
Die Aufgabe eines Autoencoders besteht darin eine neü Repräsentation einer Datenmenge
...
Neben dieser neün Repräsentation erlernt das Netzwerk noch, wie es
...
in der neün Repräsentation zwischenzuspeichern, bevor sie wieder in die ursprünliche Form gebracht werden.
...
neü Repräsenation der Features zu erlernen. Dies hat den Grund, dass die
...
Code entwickeln. Diese neü Repräsentation nennt man das \keyword{Encoding}. \\
...
den Encodingraum $\fspace{E} \subseteq \set{R}^p$, welcher die neün
...
\subsection{Neü Datenrepräsentation}
neüs Datenformat
...
\begin{minted}[frame=lines,framesep=2mm,baselinestretch=1.2,bgcolor=lightgray,fontsize=\footnotesize,linenos]{python}
  ...
  <Varname>.assign(<neür Wert>)
\end{minted}
Bei der neün Wertebesetzung kann auch der alte Wert des Tensors referenziert
...
\para{}
Man definiert meistens keine neün Operationen, sondern verwendet einfach die vordefinierten.
...
ausführt. Im folgenden Code wird das Resulat im neün Tensor
...
effizient. Deshalb wurden einerseits neü externe Hardwarebausteine, wie die
...
neü Schichten hinzugefügt werden.
...
\begin{minted}[frame=lines,framesep=2mm,baselinestretch=1.2,bgcolor=lightgray,fontsize=\footnotesize,linenos]{python}
  x_train = np.reshape(x_train, (len(x_train), 28, 28, 1)) # neü Form: (60'000, 28, 28, 1)
  x_test = np.reshape(x_test, (len(x_test), 28, 28, 1)) # neü Form: (10'000, 28, 28, 1)
\end{minted}
