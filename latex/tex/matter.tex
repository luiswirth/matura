...
In Keras kann man sehr einfach ein sogennantes \keyword{Sequential-Model} bauen.
...
\code{tf.keras.Sequential} Variabel, welcher dann mit der Methode \code{add()}
...
Dazu erstellen wir zuerst eine \code{pyplot.figure}. Innerhalb dieser haben wir
jeweils 10 \code{subplots}, fuer die verrauschten MNIST-Bilder und die
Originale. Wir zeigen dann die entsprechenden Bilder an, nachdem wir sie in Form
gebracht haben. Wir muessen der Grafik auch angeweben, dass sie schwarz-weiss
...
\code{loss} bezeichnet und die fuer den Testdatensatz mit \code{val\_loss}.
\begin{minted}[frame=lines,framesep=2mm,baselinestretch=1.2,bgcolor=lightgray,fontsize=\footnotesize,linenos]{text}
  Epoch 99/100
...
Diese muessen bei der Komilierung des Modells unter \code{metrics} angegeben
