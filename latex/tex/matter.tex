...
welchen wir in der Theorie behandelt haben, programmieren.
...
dass ihre Groesse auf $28 \times 28$ Pixel normalisiert wurde und im Bild
zentriert sind.
...
\end{minted}
Falls der Output in folgenden Charakter hat (die Versionnummern muessen nicht
...
\para{}
Nun muessen wir die Grauwerte noch normalisieren, denn im Moment liegen sie noch
...
implementieren ConvLayers erwarten die Inputs in folgender Form $(\text{Anzahl
  Inputs}, Bildbreite, Bildhoehe, Bildtiefe)$. Deshalb muessen wir mithilfe von
NumPy das Array umformen.
\begin{minted}[frame=lines,framesep=2mm,baselinestretch=1.2,bgcolor=lightgray,fontsize=\footnotesize,linenos]{python}
...
eine verrauschte Variante von ihnen. Diese werden wir nun generieren. Dazu
addieren wir einfach verschiedene Zufallswerte auf die Grauwerte der Bilder.
...
size=x_train.shape) # loc = $\mu$, scale = $\sigma^2$, size = Anzahl Samples
