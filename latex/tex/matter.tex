...
\begin{figure}
  ...
  \caption{ein Auszug an MNIST-Bildern}
\end{figure}
...
eine verrauschte Variante von ihnen. Diese werden wir nun generieren.
\para{}
Zuerst erstellen wir eine Matrix $\mat{R} \in \set{R}^{28 \times 28 \times 1}$, welche die gleiche Form, wie die
Bilder haben. Diese Matrix befuellen wir mit Zufallswerten. Fuer die
...
Die somit erhaltenen Rauschmatrizen multipliieren wir dann mit einer
...
$[0,1]$ kappen mithilfe von \code{np.clip(var, min, max)}. Diese Schritte wenden
wir sowohl fuer den Trainingsdatensatz, wie auch den Testdatensatz an. \\
...
Da wir nun die Daten haben, koennen wir unser Modell definieren.
Der einfachheitshalber werden wir vorerst unbegruendete Werte waehlen.
\para{}
Wir beginnen unser Modell mit der Inputsschicht $l=0$, welche einfach ein Tensor
ist, welcher die Inputswerte haelt.
\begin{minted}[frame=lines,framesep=2mm,baselinestretch=1.2,bgcolor=lightgray,fontsize=\footnotesize,linenos]{python}
  input_data = tf.keras.Input(shape=(28,28,1))
\end{minted}
Gefolgt wird diese von einer Reihe an Convolutional-Schichten und
Pooling-Schichten, welche sich abwechseln. Fuer die Convolutional Schicht
waehlen wir eine Filtergroesse $f^l = 3$, eine Anzahl Filter von $c^l = 32$,
einen Stride $s=1$, Same-Padding und die Aktivierungsfunktion $\varphi = \varphi^{\text{ReLU}}$.
\begin{minted}[frame=lines,framesep=2mm,baselinestretch=1.2,bgcolor=lightgray,fontsize=\footnotesize,linenos]{python}
  econv0 = tf.keras.layers.Conv2D(filters=32, kernel_size=(3,3), strides=(1,1),padding='same',activation='relu')(input_data)
\end{minted}
Fuer die Pooling-Schichten waehlen wir eine Feldgroesse $f = 2$, Stride $s = 2$ und Same-Padding.
\begin{minted}[frame=lines,framesep=2mm,baselinestretch=1.2,bgcolor=lightgray,fontsize=\footnotesize,linenos]{python}
  emaxpool0 = tf.keras.layers.MaxPooling2D(pool_size=(2,2),strides=(2,2),padding='same')(econv0)
\end{minted}
