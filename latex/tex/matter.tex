...
auf der Ebene der Netzform. Autoencoder werden verwendet um \keyword{effiziente
  Daten-Codierungen} zu erlernen und koennen so zur
...
Information codiernen.
Dies bedeutet, dass das Netzwerk erlernt die wichtigen Features (die Features
mit dem groesstem Informationsgehalt) aus den Daten zu extrahieren und nur diese
beizubehalten. \\
Neben dieser neuen Repraesentation erlernt das Netzwerk noch, wie es
aus dieser Repraesentation wieder die Ursprungsdaten reproduzieren kann, welche
dem Input sehr aehnlich sind.
\para{}
...
da nur deshalb keine Labels benoetigt werden, weil die Labels gleich den Features sind!
Somit wird das Netzwerk soll wohl darauf trainiert gewisse gewuenschte Outputs
zu liefern.
Ein Autoencoder muss also versuchen mit seinen Vorhersagen die
Features so exakt wie moeglich zu reproduzieren.

\section{Topologie}
Wie bereits erwaehnt sind die ``Labels'' eines Autoencoders gleich
den Features. Somit versucht ein Autoencoder einfach die Inputs
moeglichst exakt in die Outputs zu kopieren. Diese Operation waere an sich
ziemlich bedeutungslos und nutzlos, da man anstatt ein KNN auch einfach eine
Identitaetsfunktion verwenden koennte. Aus diesem Grund muss
man gewisse Einschraenkungen einfuehren, welche das Netzwerk dazu bringen,
interessante Methoden zu entwickeln um die Features approximativ zu
Rekonsturieren.
\para{}
Die angesprochene Einschraenkung besteht darin, dass dem Autoencoder nicht
beliebig viele Kapatzitaet fuer die Codierung der Features gegeben wird.
Es stehen dem Netzwerk nur wenige Zahlenwerte zur Verfuegung um die Features
zwischenzuspeichern, bevor sie wieder in die urspruenliche Form umgeformt werden.
Diese Kapazitaetseinschraekung wird durch die Topologie des Autoencoders erzwungen.
\para{}
Die Topologie eines Autoencoders laesst sich grob in drei Teile eingliedern:
\begin{itemize}
  \item{eine Inputschicht mit $d$ Neuronen}
  \item{eine \keyword{Flaschenhalsschicht} mit $p$ Neuronen}
  \item{eine Outputschicht mit $d$ Neuronen}
\end{itemize}
In Abbildung (\ref{fig:basic_autoencoder}) ist
ein beispielhafter Autoencoder abbgebildet, bei welchem $d = 5$ und $p = 2$
gewaehlt wurden.
\begin{figure}[h!]
  \centering
  \begin{tikzpicture}[>=latex]
...
\end{figure}
\para{}
Die Inputschicht $l=0$ enthaelt die Features $\vec{x}$. Die Outputschicht $l=2$ dagegen
enthaelt die Rekonstruktionen der Features, welche wir mit $\vec{\hat{x}}$
...
Somit ist die Flaschenhalsschicht deutlich kleiner als die anderen beiden Schichten und bildet
so die Kapazitaetseinschraenkung.
\para{}
Durch diesen Flaschenhals wird der Autoencoder gezwungen, in einem ersten Schritt die vielen Features auf
nur ein paar wenige Zahlenwerte herunterzubrechen, welche in der Zwischenschicht
platz finden. Der Autoencoder muss gewissermassen einen Code entwickeln,
mithilfe dessen er die vielen Informationen auf nur wenige wichtige
Eigenschaften komprimieren kann.
Diese neue Repraesentation der Features bezeichnet man als das
\keyword{Encoding} (deutsch: Codierung).
In einem zweiten Schritt muss der Autoencoder aus diesem Encoding wieder
versuchen die urspruenlichen Features zu rekonstruieren. Dies Rekonstruktion
bezeichnet man als das \keyword{Decoding} (deutsch: Dekodierung).
\para{}
...
Um die Funktionsweise eines Autoencoders zu verstehen, ist es hilfreich, die
Hypothesenfunktion $h$ in ein Funktionenpaar $(\phi,\psi)$ aufzuspalten:
\begin{itemize}
  \item{$\phi: \fspace{X} \to \fspace{E}$ Encoding-Funktion}
  \item{$\psi: \fspace{E} \to \fspace{X}$ Decoding-Funktion}
\end{itemize}
Die erste Teilfunktion $\phi$ ist dafuer zustaendig das Encoding zu bilden. Sie
bildet den Inputraum $\fspace{X} \subseteq \set{R}^d$, welcher die Features $\vec{x}$ enthaelt, auf
den Encodingraum $\space{E} \subseteq \set{R}^p$ ab. Weil $p$ deutlich keliner
als $d$ ist, muss der $p$-dimensionale Encodingraum $\space{E}$ eine
niedrig-dimensionale Codierung der Features darstellen. Somit kann das Encoding
nur noch die wichtigsten Merkmale der Daten, welche die ausdruckstaerksten
Eingenschaften besitzten, umfassen.

Ein Datensatz
welcher mehrere hundert Merkmale umfasst, koennte so auf nur wenige Merkmalen
heruntergebrochen werden.
Dabei geht das Rauschen der Daten, welches durch natuerliche Schwankungen der
Werte zustande kommt, verloren und ist nicht im Encoding enthalten, da es nicht
verallgemeinert werden kann.
\para{}
Nach dieser Komprimierung versucht der Autoencoder wieder aus dem
Encoding die urspruengliche Repraesentation zu rekonstruieren. Diese
Rekonstruktion bezeichnet man als das \keyword{Decoding}. Dieses Decoding ist
dem Originalinput sehr aehnlich bis auf ein paar qualitative Veluste.
\para{}
\bigskip
Mathematisch gesehen handelt es sich bei einem Autoencoder um ein Funktionenpaar
$(f,g)$. \\
Dabei ist $f: \fspace{X} \to \fspace{E}$ die sogennante
Encoder-Funktion. Sie bildet den Eingaberaum $\fspace{X} \subseteq{R}^d$, welcher die $d$-dimensionalen-Features
$\vec{x}$ enthaelt, auf den Encodingraum $\fspace{E}$, welcher die neue
Repraesentation enthaelt ab. Der Encodingraum $\fspace{E} \subseteq \set{R}^p$
ist dabei nur $p$-dimensional und es gilt: $p \ll d$. Dadurch muss das Encoding
eine niedrig-dimensionael Codierung der Features darstellen. \\
Das Gegenstueck zu $f$ ist $g: \fspace{E} \to \fspace{X}$, sie sogennante
Decoding-Funktion. Sie bildet den Encodingraum zuerueck auf den Eingaberaum ab.
\para{}
Der Output des Autoencoder bezeichnet man mit $\vec{\hat{x}}$. Er bildet sich
durch zuerstes Anwenden von $f$ auf die Features $\vec{x}$ und anschliessendes
Anwenden der Funktion $g$ auf das Resultat.
\begin{equation}
  \vec{\hat{x}} = g(f(\vec{x})) = (g \circ f)(\vec{x})
\end{equation}
Die Aufgabe des Autoencoder besteht nun darin, diese beiden Funktionen $f$ und
$g$ zu erlernen. Dabei soll folgende Bedingung gelten (falls Mittlerer Quadratischer Fehler)
\begin{equation}
  \min_{f,g} (\vec{x} - \vec{\hat{x}})^2
\end{equation}



Gesucht werden nun die Funktionen.
Der Output $\vec{\hat{x}}$ eines Autoencoders bildet man durch die Anwendung der
Funktion $g$ auf das Ergebniss der Anwendung von $f$ auf ein beliebiges Element
$\vec{x} \in \fspace{X}$ des Eingaberaums.
\para{}
Nun stellt man folgende Bedingung an die Funktionen $f$ und $g$.

Fuer alle $\vec{x} \in \fspace{X}$ soll gelten:
Daraus laesst sich lesen, dass Bedeutung: Bei Anwendung der Funktion $g$ auf das Ergebniss der Anwendung von
$f$ auf ein beliebiges Elemente $\vec{x}$ des Eingaberaums, optimalerweise genau das
urspruengliche Element $\vec{\hat{x}} = \vec{x}$ erhaelt. Dieses Verhalten wird
durch die Minimierung des Fehlers zwischen $\vec{x}$ und $\vec{\hat{x}}$ gefordert. Da $p$
kleiner als $d$ ist muss $\fspace{F}$ offentsichtlich eine niedrig-dimensionale Codierung
von $\fspace{X}$ darstellen.
Optimal: $\hat{x} = x$ aber nicht moeglich, daher eigentlich $g: \fspace{F} \to \fspace{D}$


Der Trainingsdatensatz ist der Eingaberaum $X$.


Ein Autoencoder zeichnet sich durch seine Netzform aus. Diese sieht aehnlich zu
einem Doppelkegel aus.
Man kann einen Autoencoder grob in fuenf Teile eingliedern:
\begin{itemize}
\item{\keyword{Eingabeschicht}}
\item{\keyword{Encoder}}
\item{\keyword{Flaschenhals} mit der \keyword{Encoding}-Repraesentation}
\item{\keyword{Decoder}}
\item{\keyword{Ausgabeschicht} mit der \keyword{Decoding}-Repraesentation}
\end{itemize}
...
Visualisierung als 2D-Daten oder 3D-Daten, Kompression braucht nur noch Anteil von
$\frac{p}{d}$ des urspruenglichen Speicherbedarfs. Diese Komprimierung ist dabei Datenspezifisch.

\section{Convolutional-Autoencoder}
Convolutional-, Pooling- und Upsampling-Schichten anstatt Fully-Connected.
