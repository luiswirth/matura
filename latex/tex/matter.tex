...
\footnote{Üblicherweise wird auf Gymnasialstufe vermittelt, dass die lokalen
  Extrema (inklusive lokalen Minimas) einer Funktion $f$ bestimmt werden
  bestimmen, indem die erste Ableitung $f'$ gebildet und gleich null gesetzt
  wird. Dies ist hier jedoch nicht möglich, da die Funktion
  $C'(\param_1,\param_2,\ldots,\param_n)$ zu komplex ist, um die
...
gehen, ein spezifisches Modell, namentlich Künstliche Neuronale Netze, zu erläutern.
\para{}
...
Spracherkennung.
Vorallem aber für hochauflösende Bilder sind die KNNs, die wir soeben
...
den sogenannten \keyword{Filtertensor} $\ten{F}$ oder Faltungstensor (siehe Abb.
...
Element, der sogenannte \keyword{Zentralelement} (engl.: Center element), bezeichnet mit $(\ten{F})_C$,
