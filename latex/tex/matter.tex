...
\para{}
\begin{examplebox}{Beispiel für Daten}
Um die Begriffe zu verdeutlichen, folgt nun ein Beispiel.
...
mehreren Samples, wobei jedes Sample Werte, welche von einer Person gemessen
wurden, enthält. Ein Sample beinhaltet beispielsweise die Inputs
$\vec{x} = \trans{\begin{pmatrix} \SI{18}{\year} & \SI{65}{\kg} \end{pmatrix}}$
und die Labels
$\vec{\hat{y}} = \trans{\begin{pmatrix} \SI{180}{\centi\metre} \end{pmatrix}}$
umfassen. Das trainierte Modell erzeugt für die Inputs diese Sampels die
...
Ein Modell soll verwendet werden, um den Zusammenhang zwischen dem
Benzinverbrauch eines Autos und der zurückgelegten Strecke zu erlernen.
Dafür wird angenommen, das es sich dabei um eine lineare Beziehung zwischen
diesen Variablen handelt. Somit eignet sich das wohl einfachste
Regressionmodell: eine Regressiongerade der From $y=\param_1x + \param_0$, wobei
...
Hypothesenfunktion besitzten $y = h(x) = 0.07 \cdot x + 0$.
\end{examplebox}
\para{}
...
Aktivierungsfunktion in KNNs. Sie weichst stark von der Linearität ab und ist
...
Abbildungen \refbox{fig:nomenklatur1} und \refbox{fig:nomenklatur2} zum
...
In Abbildung \refbox{fig:edge_filter} wurde der erwähnte Kantendetektionfilter auf ein
...
Betrachten wird der
allgemeinen Bildtensor $\ten{B} \in \set{R}^{h \times w \times c}$, wobei $c$ die Anzahl
...
Nasenbreite, zur Augenfarbe usw. als Code verwenden, welche deutlich geringeren
...
Wir werden nun zwei Anwendungen grob betrachten und eine Anwendung wird in der
...
werden. Es handelt sich also um \keyword{verlustbehaftete Kompression}.
...
Auch hier wieder ist PCA eine alternative Methode zum Autoencoder.
...
mutlipliziert, welcher die Rauschstärke bestimmt. Das Resultate wird zum
...
\subsection{Daten}
Als Trainingsdaten wird der \keyword{MNIST}-Datensatz verwendet. Es handlet sich
...
\code{'glorot\_normal'} gesetzt werden. Dadurch findet die Initalisierung analog
zu Sektion \refbox{sec:parameter_initalisieren} statt. \\
...
\end{minted}
Die Kostenfunktion weist bezüglich dem Testdatensatz nach dem Training einen Wert \code{val\_los} von $0.0180$,
auf.
\para{}
...
\keyword{Kreuzentropie-Kostenfunktion} als deutlich raffinierter ist als die MSE-Kostenfunktion.
