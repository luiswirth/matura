...
(engl.: computation) durch einen gerichteten Graphen beschreibt. Zuest stellt
...
\para{}
Jeder tf.Tensoren ist mit einer einzigen Graph-Ausfuehrung assoziert. Sie
besitzen ihren Wert nur fuer den jeweiligen Durchlauf. Man sagt die Tensoren sind ``immutable''.
\para{}
Die meisten Tensoren sind nur Zwischenwerte der Graphenberechnung, bzw. einfach
eine Durchflussstelle. Daneben gibt es auch noch einige spezielle Tensoren, die
ein spezifisches Verhalten aufweisen. Diese wichtigsten dieser Art, werden wir
nun betrachten.
\paragraph{tf.constant}
Der \keycode{tf.Constant} Tensor ist ein Knoten innerhalb des Graphs, welcher immer einen
konstanten Wert als Ouput hat. Er hat somit keine Inputs.
\para{}
...
Moeglichkeit ihn mit Inputs zu befuettern. Dafuer gibt es den sogennanten
\keycode{tf.Placeholder} Tensor. Bei der Werte-Einspeisung wird er mit seinem
Variabelnnamen refenziert. Man muss seine Dimensionen und seinen Datentyp bei der Erstellung angeben.
Er hat logischerweise auch nur Outputs und keinerlei Inputs.
\para{}
Ihn definiert man folgenderweise:
\begin{minted}[frame=lines,framesep=2mm,baselinestretch=1.2,bgcolor=lightgray,fontsize=\footnotesize,linenos]{python}
  input = tf.placeholder(dtype=tf.float32, shape=(2,2))
\end{minted}
...
koennen modifiziert werden. Somit sind sie geeignet, fuer Modellparameter. Diese
sind muessen auch grundsaetzlich erhalten bleiben und nur beim Trainingvorgang
angepasst werden.
\para{}
Folgender Python-Code kreeirt eine tf.Varaible und aendert ihren Wert danach.
\begin{minted}[frame=lines,framesep=2mm,baselinestretch=1.2,bgcolor=lightgray,fontsize=\footnotesize,linenos]{python}
...
Ein Beispiel dafuer waere eine Matrix-Multiplikation (in TF als
``MatMul'' bezeichnet). Zum Teil besizten diese Operationen gewisse Attribute,


welche sie genauer definieren, dies ist zum Beispiel der Fall bei einer
Faltungsoperation, bei welcher man den Stride und das Padding angeben kann.
\para{}
Nun koennen wir einen eigentlichen Graphen auch definieren, denn dafuer muss man
in TensorFlow lediglich die Operationen definieren und gleichzeitig verketten.
Die Knotenpunkte werden von TensorFlow automatisch erstellt.
\para{}
Nun folgt ein Codebeispiel fuer die Erstellung eines Graphens mit mehren Operationen.
\begin{minted}[frame=lines,framesep=2mm,baselinestretch=1.2,bgcolor=lightgray,fontsize=\footnotesize,linenos]{python}
  c = tf.constant([[1.0, 2.0], [3.0, 4.0]])
  d = tf.constant([[1.0, 1.0], [0.0, 1.0]])
  e = tf.matmul(c, d)
\end{minted}
Zuest werden zwei konstante Tensoren 2.Ordnung \mintinline{python}{c} und \mintinline{python}{d}
erstellt. Dann wird eine Matrixmultiplikations-Operation zwischen ihnen
definiert, welche als Output den Tensor \mintinline{python}{e} befuellt.
...
Operation- und Tensor-Objekte.
Sie erwartet eine List von Outputtensoren, in welche die Resultate der Berechnung
reingeschrieben werden sollen.
\para{}
Nun koennen wir auch unseren Graph ausfuehren, denn dieser wird zusammen mit der
Session ausgefuehrt. Dies macht man folgendermassen:
\begin{minted}[frame=lines,framesep=2mm,baselinestretch=1.2,bgcolor=lightgray,fontsize=\footnotesize,linenos]{python}
  sess = tf.Session()
  result = sess.run()
\end{minted}
...
\subsubsection{Beispiel: Perzeptron in TF}
