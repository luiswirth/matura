...
Viele Ideen bezüglich dieser Arbeit konnten aufgrund des bereits jetzt sehr
grossen Umfangs nicht realisiert werden. Es war ursprünglich eine deutlich
mächtigere Problemstellung zu lösen angedacht gewesen.
Aufgrund dieser Reduktion des Anspruches, wurde das eigentliche Potenzial von
TensorFlow und Keras bei weitem nicht ausgeschöpft. Dadurch könnte in falscher
Eindruck entstehen. Deshalb ist es essentiell zu erkennen, dass TensorFlow und
Keras zu weitem mehr im Stande sind.
\para{}
Um noch einige weitere Konzepte des Maschinellen Lernen, welche teilweise für
diese Arbeit intendiert waren, folgt nun ein Ausblick über sonstige Aspekte.

\section*{Technische Aspekte}
\subsection*{Weitere Kostenfunktionen}
Cross-entropy.
\subsection*{Weitere Optimisierungsverfahren}
Adam-Optimizer
Momentum-Optimizer
...
Wie bereits im Vorwort erwähnt, war es der ursprüngliche Plan dieser Arbeit
einen \keyword{Variational Autoencoder} (VAE) zu implementieren. Dieser sollte genutzt werden,
um künstliche generierte Bilder von menschlichen Gesichtern zu erzeugen.
Dies ist möglich, da es sich bei einem VAE um ein sogennantes
\keyword{Generatives Modell} handelt.
\para{}
Generative Modelle sind im Allgemeinen Modelle, welche in der Lage sind anhand
von einem existierenden Datensatz neue Dateneinträge zu generieren. Diese neuen Daten
weisen einen ähnlichen Charakter auf, wie der ursprüngliche Datensatz, sind
jedoch nicht so in ihm enthalten.
\para{}
Ein VAE baut das Grundkonzept eines normalen Autoencoders aus. Er verwandelt das
Encoding (Repräsentation) in eine Stochastische Verteilung, welche den Code enthält. Dadurch wird
bezwegt, dass Codes, welche nahe beiander sind, auch nahe beieinander liegede
Decodings (Rekonstruktionen) erzeugt. Falls ein VAE mit menschlichen Gesichtern
trainiert wird, können neue nie-gesehene Codes rekonstruiert werden und so neue
Gesichter erhalten.

\section*{Generative Adversial Networks}
Ein weiteres Generatives Modell sind sogennante \keyword{Generative Adversial
  Networks} (GANs). Sie sind der momentane Status Quo, was generative KNNs
anbelangt.
\para{}
Der Grundkonzept ihrer Funktionsweise lautet wie folgt. Ein GAN besteht aus zwei
KNNs, welche miteinander konkurrenzieren. Das eine Modell, der
...
dem Trainingsdatensatz ähneln. Der Diskriminator hingegen bekommt immer entweder
ein Sample aus dem Trainingsdatensatz vorgelegt oder ein generiertes Sample und
muss dann entscheiden, ob es echt oder generiert ist. Beide Netzwerke lernen so,
...
wachst gewissermassen aus seinen Kinderschuhen. Es handelt sich dabei um das
...
völlig stabil. Sobald TF 2.0 offiziel freigegeben wird, ist es zu empfehlen
davon Gebrauch zu machen. In dieser Überarbeitung werden viele Relikte aus den
ursprünglichen TensorFlow Versionen entfernt, welche zu Verwirrung sorgen
können. Somit entsteht ein in sich stimmiges Framework, welches wirklich
angenehm zu bedienen ist.

\subsection*{Alternativen}
Es gibt neben TensorFlow und Keras noch weitere Frameworks, welche sich für
Maschinelles Lernen eignen. Die meisten davon sind auch hauptsächlich auf Deep
Learning ausgelegt. Sie machen meistens auch Gebrauch von der cuDNN-Bibliothek
von NVIDIA (erwähnt in Anhang (\ref{sec:anhang_tf})). Somit weisen sie mehr oder
weniger gleich performante Implementierungen wie TF auf. Vorteile können in der
Intuition und Einfachheit der Bedienung liegen. Viele Privatpersonen bevorzugen
genau aus diesem Grund \keyword{PyTorch} gegenüber TF.
